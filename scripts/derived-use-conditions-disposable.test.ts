...
import pg from "pg";
test("Derived Use Condition authoring saves and evaluates against a disposable PostgreSQL cluster", { timeout: 1_500_000 }, async () => {
  const parent = path.resolve(tmpdir()), root = path.resolve(await mkdtemp(path.join(parent, "serrian-derived-conditions-")));
  assert.equal(path.dirname(root), parent); assert.ok(path.basename(root).startsWith("serrian-derived-conditions-"));
  ...
  const databaseUrl = `postgresql://postgres@127.0.0.1:${port}/serrian_derived_conditions_dev`;
  ...
  try {
    ...
    pool = new pg.Pool({ connectionString: `postgresql://postgres@127.0.0.1:${port}/postgres` }); await pool.query("create database serrian_derived_conditions_dev"); await pool.end();
    ...
    execFileSync(process.execPath, ["--conditions=react-server", "--import", "tsx", "scripts/derived-use-conditions-browser.ts"], { cwd: process.cwd(), windowsHide: true, stdio: "inherit", timeout: 1_400_000,
      env: { ...environment, DATABASE_URL: databaseUrl, NODE_ENV: "test", SERRIAN_DISPOSABLE_DERIVED_CONDITIONS: "true" } });
  } finally {
