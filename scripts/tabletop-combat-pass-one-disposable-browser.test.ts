...
async function findLoopbackPort(): Promise<number> {
  ...
  if (!port) throw new Error("A disposable Pass 1 browser PostgreSQL port could not be reserved.");
  return port;
}

test("Pass 1 G.O.D. and Player browser flows use a disposable migrated PostgreSQL cluster", { timeout: 900_000 }, async () => {
  const temporaryCluster = await mkdtemp(path.join(tmpdir(), "serrian-combat-pass-one-browser-postgres-"));
  ...
  const databaseUrl = `postgresql://postgres@127.0.0.1:${port}/serrian_combat_pass_one_browser_dev`;
  ...
  try {
    ...
    await pool.query("create database serrian_combat_pass_one_browser_dev");
    await pool.end();
    ...
    for (const script of [
      "scripts/action-declaration-windows-browser.test.ts",
      "scripts/player-combat-console-browser.test.ts",
      "scripts/player-tabletop-console-browser.test.ts",
    ]) {
