...
export async function capturePaperPdf(page:Page,label:string,expected:{name:string;campaign:string}) {
  ...
  const manifest={...expected,paragraphs,rows,checks:paragraphs,rowIds:[],rowCount:rows.length};
...
export async function reviewSavedPaperCharacter(pool:pg.Pool,login:(email:string)=>Promise<Page>,baseUrl:string,review:Awaited<ReturnType<typeof seedPaperReview>>) {
  ...
  const grasp=await root.locator('.paper-spell-quick tbody tr').filter({hasText:'Charged Grasp'}).locator('td').allTextContents();
  assert.deepEqual(grasp.slice(1,3),['4','2'],'Current Novice casting cost and Initiative must match the authoritative preview');
  ...
  await page.getByLabel('Spell / ability play references',{exact:true}).check();
  await page.getByRole('button',{name:'Print / Save as PDF',exact:true}).click();
  await page.waitForFunction(()=>document.documentElement.dataset.paperPrintCalls==='2');
  assert.equal(await root.locator('.paper-spell').count(),s.paper.spells.length);
