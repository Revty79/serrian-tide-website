...
export function CombatRollPanel({ prompts, onSubmit, emptyMessage, unavailableReason, rolls = [] }: {
  ...
  const blocked = !prompt?.ready || Boolean(unavailableReason && prompt?.kind !== "free") || submittedKeys.has(key);
  ...
  async function submit(method: "random" | "entered") {
    ...
    try {
      ...
      if (prompt.manualTargetRequired) {
        if (!draft.label.trim() || !draft.target.trim() || !Number.isFinite(Number(draft.target))) throw new Error("Enter the G.O.D. ruling label and a finite Roll target for this action.");
        input.manualLabel = draft.label.trim(); input.manualTarget = Number(draft.target);
      }
    } catch (error) {
    ...
    try {
      ...
      // Keep every submitted Roll locked across selection changes and stale refreshes.
      // Finishing a firearm is an idempotent continuation, NOT another Roll slot:
      // a response may still be outstanding on the server, so it must remain retryable.
      if (prompt.kind !== "free" && prompt.kind !== "firearm-finish") {
        setSubmittedKeys((current) => new Set(current).add(prompt.key));
      }
  ...
  return <BattleStage eyebrow="DICE" title="Combat rolls" detail="Roll here without leaving combat. Choose the named action or defense to resolve it; Other d100 is only a general Roll.">
    <div aria-busy={busy} data-combat-roll-kind={prompt?.kind} data-combat-record-id={prompt?.recordId}>
      {prompts.length > 1 ? <label className="st-field" htmlFor={`${inputId}-choice`}><span>Roll for</span>
      ...
      <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", alignItems: "end" }}>
        {continuation ? <button className="st-button is-primary" type="button" disabled={busy || blocked} onClick={() => void submit("random")}>
          {prompt?.kind === "firearm-trigger" ? "Pull trigger" : "Finish firing — use recorded Roll"}
        </button> : <>
          ...
          <label className="st-field" htmlFor={`${inputId}-physical`}><span>Physical Roll (1–100; 00 = 100)</span>
            <input id={`${inputId}-physical`} className="st-control" aria-label={`Physical ${prompt?.kind === "firearm-roll" ? "firearm" : prompt?.kind ?? "combat"} Roll`} inputMode="numeric" pattern="[0-9]{1,3}" placeholder="01–99, 00 or 100" disabled={busy || blocked} value={draft.entered} onChange={(event) => changeDraft({ entered: event.target.value })} />
          </label>
      ...
      {submittedKeys.has(key) ? <p role="status">Recorded for this action. Refreshing combat; this Roll will not be submitted again.</p> : null}
