...
export const campaignSessionEncounterFirearmAttack = pgTable(
  ...
  (table) => [
    ...
    check("campaign_session_encounter_firearm_attack_fire_state_valid", sql`(${table.firedAt} IS NULL AND ${table.firedByUserId} IS NULL AND ${table.attackRollId} IS NULL AND ${table.roundsConsumed} = 0 AND ${table.roundsLoadedAfter} IS NULL) OR (${table.firedAt} IS NOT NULL AND ${table.firedByUserId} IS NOT NULL AND ${table.attackRollId} IS NOT NULL AND ${table.roundsConsumed} = ${table.roundsDeclared} AND ${table.roundsLoadedAfter} = ${table.roundsLoadedBefore} - ${table.roundsConsumed})`),
