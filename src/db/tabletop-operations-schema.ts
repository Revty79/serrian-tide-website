...
export const campaignSessionEncounterEffectPlan = pgTable(
  "campaign_session_encounter_effect_plan",
  {
    ...
    declarationId: integer("declaration_id").notNull(),
    pendingActionId: integer("pending_action_id").notNull(),
    firearmPortion: integer("firearm_portion").default(0).notNull(),
    encounterId: integer("encounter_id").notNull(),
    sceneId: integer("scene_id").notNull(),
    sessionId: integer("session_id").notNull(),
    campaignId: integer("campaign_id").notNull(),
  ...
  (table) => [
    uniqueIndex("campaign_session_encounter_effect_plan_declaration_uq").on(table.declarationId, table.firearmPortion),
    uniqueIndex("campaign_session_encounter_effect_plan_pending_action_uq").on(table.pendingActionId, table.firearmPortion),
    check("campaign_session_encounter_effect_plan_portion_valid", sql`${table.firearmPortion} >= 0`),
...
export const campaignSessionEncounterFirearmAttack = pgTable(
  "campaign_session_encounter_firearm_attack",
  {
    ...
    itemId: integer("item_id").notNull(),
    weaponProfileId: integer("weapon_profile_id").notNull(),
    firingModeId: integer("firing_mode_id").notNull(),
    ammunitionItemId: integer("ammunition_item_id").notNull(),
    ammunitionProfileId: integer("ammunition_profile_id").notNull(),
    aimDeclarationId: integer("aim_declaration_id"),
    aimPendingActionId: integer("aim_pending_action_id"),
    triggerDeclarationId: integer("trigger_declaration_id").notNull(),
    triggerPendingActionId: integer("trigger_pending_action_id"),
    attackRollId: integer("attack_roll_id"),
    effectPlanId: integer("effect_plan_id"),
    status: text("status").notNull(),
    stateVersionBefore: integer("state_version_before").notNull(),
    aimInitiative: integer("aim_initiative").default(0).notNull(),
    calledShotDeclared: boolean("called_shot_declared").default(false).notNull(),
    calledShotObjective: text("called_shot_objective").default("").notNull(),
    calledShotLocationNumber: integer("called_shot_location_number"),
    calledShotPenalty: doublePrecision("called_shot_penalty"),
    calledShotReason: text("called_shot_reason").default("").notNull(),
    firingDurationInitiative: integer("firing_duration_initiative").default(1).notNull(),
    roundsPerCadence: integer("rounds_per_cadence").notNull(),
    roundsDeclared: integer("rounds_declared").notNull(),
    roundsConsumed: integer("rounds_consumed").default(0).notNull(),
    firingPortionsResolved: integer("firing_portions_resolved").default(0).notNull(),
    roundsLoadedBefore: integer("rounds_loaded_before").notNull(),
    roundsLoadedAfter: integer("rounds_loaded_after"),
    finalTarget: doublePrecision("final_target").notNull(),
    frozenSnapshotJson: jsonb("frozen_snapshot_json").notNull(),
    governingSnapshotJson: jsonb("governing_snapshot_json").notNull(),
    attackRollSnapshotJson: jsonb("attack_roll_snapshot_json"),
    defenseResolutionJson: jsonb("defense_resolution_json"),
    bulletAllocationJson: jsonb("bullet_allocation_json"),
    damageResolutionJson: jsonb("damage_resolution_json"),
    postShotStateJson: jsonb("post_shot_state_json"),
    rulingReasonsJson: jsonb("ruling_reasons_json").default(sql`'[]'::jsonb`).notNull(),
    idempotencyKey: text("idempotency_key").notNull(),
    createdByUserId: text("created_by_user_id").notNull(),
    firedByUserId: text("fired_by_user_id"),
  ...
  (table) => [
    foreignKey({
      ...
      name: "campaign_session_encounter_firearm_attack_encounter_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.encounterId, table.sceneId, table.sessionId, table.campaignId, table.actorParticipantId],
      ...
      name: "campaign_session_encounter_firearm_attack_actor_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.encounterId, table.sceneId, table.sessionId, table.campaignId, table.targetParticipantId],
      foreignColumns: [campaignSessionEncounterParticipant.encounterId, campaignSessionEncounterParticipant.sceneId, campaignSessionEncounterParticipant.sessionId, campaignSessionEncounterParticipant.campaignId, campaignSessionEncounterParticipant.characterId],
      name: "campaign_session_encounter_firearm_attack_target_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.itemInstanceId, table.campaignId, table.actorParticipantId, table.itemId, table.weaponProfileId],
      foreignColumns: [campaignCharacterFirearmState.itemInstanceId, campaignCharacterFirearmState.campaignId, campaignCharacterFirearmState.characterId, campaignCharacterFirearmState.itemId, campaignCharacterFirearmState.weaponProfileId],
      name: "campaign_session_encounter_firearm_attack_state_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.firingModeId, table.weaponProfileId],
      foreignColumns: [weaponFiringMode.id, weaponFiringMode.weaponProfileId],
      name: "campaign_session_encounter_firearm_attack_mode_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.ammunitionProfileId, table.ammunitionItemId],
      foreignColumns: [weaponProfile.id, weaponProfile.itemId],
      name: "campaign_session_encounter_firearm_attack_ammunition_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.aimDeclarationId, table.encounterId, table.sceneId, table.sessionId, table.campaignId],
      ...
      name: "campaign_session_encounter_firearm_attack_aim_declaration_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.aimPendingActionId, table.encounterId, table.sceneId, table.sessionId, table.campaignId],
      foreignColumns: [campaignSessionEncounterPendingAction.id, campaignSessionEncounterPendingAction.encounterId, campaignSessionEncounterPendingAction.sceneId, campaignSessionEncounterPendingAction.sessionId, campaignSessionEncounterPendingAction.campaignId],
      name: "campaign_session_encounter_firearm_attack_aim_pending_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.triggerDeclarationId, table.encounterId, table.sceneId, table.sessionId, table.campaignId],
      foreignColumns: [campaignSessionEncounterActionDeclaration.id, campaignSessionEncounterActionDeclaration.encounterId, campaignSessionEncounterActionDeclaration.sceneId, campaignSessionEncounterActionDeclaration.sessionId, campaignSessionEncounterActionDeclaration.campaignId],
      name: "campaign_session_encounter_firearm_attack_trigger_declaration_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.triggerPendingActionId, table.encounterId, table.sceneId, table.sessionId, table.campaignId],
      foreignColumns: [campaignSessionEncounterPendingAction.id, campaignSessionEncounterPendingAction.encounterId, campaignSessionEncounterPendingAction.sceneId, campaignSessionEncounterPendingAction.sessionId, campaignSessionEncounterPendingAction.campaignId],
      name: "campaign_session_encounter_firearm_attack_trigger_pending_fk",
    }).onDelete("restrict"),
    foreignKey({ columns: [table.attackRollId], foreignColumns: [campaignSessionRoll.id], name: "campaign_session_encounter_firearm_attack_roll_fk" }).onDelete("restrict"),
    foreignKey({ columns: [table.effectPlanId], foreignColumns: [campaignSessionEncounterEffectPlan.id], name: "campaign_session_encounter_firearm_attack_effect_plan_fk" }).onDelete("restrict"),
    foreignKey({ columns: [table.createdByUserId], foreignColumns: [user.id], name: "campaign_session_encounter_firearm_attack_created_by_fk" }).onDelete("restrict"),
    foreignKey({ columns: [table.firedByUserId], foreignColumns: [user.id], name: "campaign_session_encounter_firearm_attack_fired_by_fk" }).onDelete("restrict"),
    foreignKey({ columns: [table.cancelledByUserId], foreignColumns: [user.id], name: "campaign_session_encounter_firearm_attack_cancelled_by_fk" }).onDelete("restrict"),
    uniqueIndex("campaign_session_encounter_firearm_attack_idempotency_uq").on(table.campaignId, table.idempotencyKey),
    uniqueIndex("campaign_session_encounter_firearm_attack_trigger_declaration_uq").on(table.triggerDeclarationId),
    uniqueIndex("campaign_session_encounter_firearm_attack_aim_declaration_uq").on(table.aimDeclarationId).where(sql`${table.aimDeclarationId} IS NOT NULL`),
    uniqueIndex("campaign_session_encounter_firearm_attack_roll_uq").on(table.attackRollId).where(sql`${table.attackRollId} IS NOT NULL`),
    uniqueIndex("campaign_session_encounter_firearm_attack_effect_plan_uq").on(table.effectPlanId).where(sql`${table.effectPlanId} IS NOT NULL`),
    uniqueIndex("campaign_session_encounter_firearm_attack_one_open_instance_uq").on(table.itemInstanceId).where(sql`${table.status} IN ('aiming','trigger-ready','committed')`),
    unique("campaign_session_encounter_firearm_attack_hierarchy_uq").on(table.id, table.encounterId, table.sceneId, table.sessionId, table.campaignId),
    index("campaign_session_encounter_firearm_attack_history_idx").on(table.encounterId, table.createdAt, table.id),
    index("campaign_session_encounter_firearm_attack_actor_idx").on(table.actorParticipantId, table.createdAt, table.id),
    check("campaign_session_encounter_firearm_attack_status_valid", sql`${table.status} IN ('aiming','trigger-ready','committed','fired-awaiting-timing','consequence-planned','requires-god-ruling','cancelled')`),
    check("campaign_session_encounter_firearm_attack_identity_distinct", sql`${table.actorParticipantId} > 0 AND ${table.targetParticipantId} <> ${table.actorParticipantId}`),
    check("campaign_session_encounter_firearm_attack_state_version_positive", sql`${table.stateVersionBefore} > 0`),
    check("campaign_session_encounter_firearm_attack_aim_nonnegative", sql`${table.aimInitiative} >= 0`),
    check("campaign_session_encounter_firearm_attack_called_shot_valid", sql`(NOT ${table.calledShotDeclared} AND ${table.calledShotObjective} = '' AND ${table.calledShotLocationNumber} IS NULL AND ${table.calledShotPenalty} IS NULL AND ${table.calledShotReason} = '') OR (${table.calledShotDeclared} AND length(trim(${table.calledShotObjective})) > 0 AND ${table.calledShotPenalty} >= 0 AND length(trim(${table.calledShotReason})) > 0)`),
    check("campaign_session_encounter_firearm_attack_rounds_positive", sql`${table.firingDurationInitiative} > 0 AND ${table.roundsPerCadence} > 0 AND ${table.roundsDeclared} > 0`),
    check("campaign_session_encounter_firearm_attack_consumption_valid", sql`${table.roundsConsumed} >= 0 AND ${table.roundsConsumed} <= ${table.roundsDeclared} AND ${table.roundsLoadedBefore} >= ${table.roundsDeclared} AND (${table.roundsLoadedAfter} IS NULL OR ${table.roundsLoadedAfter} >= 0)`),
    check("campaign_session_encounter_firearm_attack_aim_identity_valid", sql`(${table.aimInitiative} = 0 AND ${table.aimDeclarationId} IS NULL AND ${table.aimPendingActionId} IS NULL) OR (${table.aimInitiative} > 0 AND ${table.aimDeclarationId} IS NOT NULL AND ${table.aimPendingActionId} IS NOT NULL)`),
    check("campaign_session_encounter_firearm_attack_trigger_identity_valid", sql`(${table.triggerPendingActionId} IS NULL AND ${table.status} IN ('aiming','trigger-ready','cancelled')) OR (${table.triggerPendingActionId} IS NOT NULL AND ${table.status} NOT IN ('aiming','trigger-ready'))`),
    check("campaign_session_encounter_firearm_attack_fire_state_valid", sql`(${table.firedAt} IS NULL AND ${table.firedByUserId} IS NULL AND ${table.attackRollId} IS NULL AND ${table.roundsConsumed} = 0 AND ${table.roundsLoadedAfter} IS NULL) OR (${table.firedAt} IS NOT NULL AND ${table.firedByUserId} IS NOT NULL AND ${table.attackRollId} IS NOT NULL AND ${table.roundsConsumed} > 0 AND ${table.roundsConsumed} <= ${table.roundsDeclared} AND ${table.roundsLoadedAfter} = ${table.roundsLoadedBefore} - ${table.roundsConsumed})`),
    check("campaign_session_encounter_firearm_attack_portions_valid", sql`${table.firingPortionsResolved} >= 0 AND ${table.firingPortionsResolved} <= ${table.firingDurationInitiative}`),
    check("campaign_session_encounter_firearm_attack_cancel_state_valid", sql`(${table.status} = 'cancelled' AND ${table.cancelledAt} IS NOT NULL AND ${table.cancelledByUserId} IS NOT NULL) OR (${table.status} <> 'cancelled' AND ${table.cancelledAt} IS NULL AND ${table.cancelledByUserId} IS NULL)`),
    check("campaign_session_encounter_firearm_attack_snapshot_objects", sql`jsonb_typeof(${table.frozenSnapshotJson}) = 'object' AND jsonb_typeof(${table.governingSnapshotJson}) = 'object' AND (${table.attackRollSnapshotJson} IS NULL OR jsonb_typeof(${table.attackRollSnapshotJson}) = 'object') AND (${table.defenseResolutionJson} IS NULL OR jsonb_typeof(${table.defenseResolutionJson}) = 'object') AND (${table.bulletAllocationJson} IS NULL OR jsonb_typeof(${table.bulletAllocationJson}) = 'object') AND (${table.damageResolutionJson} IS NULL OR jsonb_typeof(${table.damageResolutionJson}) = 'object') AND (${table.postShotStateJson} IS NULL OR jsonb_typeof(${table.postShotStateJson}) = 'object') AND jsonb_typeof(${table.rulingReasonsJson}) = 'array'`),
    check("campaign_session_encounter_firearm_attack_idempotency_nonblank", sql`length(trim(${table.idempotencyKey})) > 0`),
  ],
);

export const campaignSessionEncounterFirearmBullet = pgTable(
  "campaign_session_encounter_firearm_bullet",
  {
    ...
    status: text("status").notNull(),
    cancelledByReactionId: integer("cancelled_by_reaction_id"),
    hitLocationNumber: integer("hit_location_number"),
    hitLocationName: text("hit_location_name").default("").notNull(),
    hpPoolKey: text("hp_pool_key").default("").notNull(),
    authoredDamage: doublePrecision("authored_damage"),
    dexDamageModifier: doublePrecision("dex_damage_modifier").default(0).notNull(),
    additionalSuccessDamage: integer("additional_success_damage").default(0).notNull(),
    grossDamage: doublePrecision("gross_damage"),
    armor: doublePrecision("armor"),
    soak: doublePrecision("soak"),
    proposedNetDamage: doublePrecision("proposed_net_damage"),
    armorSnapshotJson: jsonb("armor_snapshot_json").notNull(),
    rulingReasonsJson: jsonb("ruling_reasons_json").default(sql`'[]'::jsonb`).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (table) => [
    foreignKey({
      columns: [table.attackId, table.encounterId, table.sceneId, table.sessionId, table.campaignId],
      ...
      name: "campaign_session_encounter_firearm_bullet_attack_fk",
    }).onDelete("restrict"),
    foreignKey({ columns: [table.cancelledByReactionId], foreignColumns: [campaignSessionEncounterReaction.id], name: "campaign_session_encounter_firearm_bullet_reaction_fk" }).onDelete("restrict"),
    uniqueIndex("campaign_session_encounter_firearm_bullet_order_uq").on(table.attackId, table.bulletIndex),
    index("campaign_session_encounter_firearm_bullet_attack_idx").on(table.attackId, table.status, table.bulletIndex),
    check("campaign_session_encounter_firearm_bullet_index_positive", sql`${table.bulletIndex} > 0`),
    check("campaign_session_encounter_firearm_bullet_status_valid", sql`${table.status} IN ('cancelled-by-defense','surviving','requires-god-ruling')`),
    check("campaign_session_encounter_firearm_bullet_damage_nonnegative", sql`(${table.authoredDamage} IS NULL OR ${table.authoredDamage} >= 0) AND ${table.additionalSuccessDamage} >= 0 AND (${table.grossDamage} IS NULL OR ${table.grossDamage} >= 0) AND (${table.armor} IS NULL OR ${table.armor} >= 0) AND (${table.soak} IS NULL OR ${table.soak} >= 0) AND (${table.proposedNetDamage} IS NULL OR ${table.proposedNetDamage} >= 0)`),
    check("campaign_session_encounter_firearm_bullet_snapshot_valid", sql`jsonb_typeof(${table.armorSnapshotJson}) = 'object' AND jsonb_typeof(${table.rulingReasonsJson}) = 'array'`),
  ],
);

export const campaignSessionEncounterFirearmAttackEvent = pgTable(
  ...
  (table) => [
    foreignKey({
      columns: [table.attackId, table.encounterId, table.sceneId, table.sessionId, table.campaignId],
      foreignColumns: [campaignSessionEncounterFirearmAttack.id, campaignSessionEncounterFirearmAttack.encounterId, campaignSessionEncounterFirearmAttack.sceneId, campaignSessionEncounterFirearmAttack.sessionId, campaignSessionEncounterFirearmAttack.campaignId],
      name: "campaign_session_encounter_firearm_attack_event_attack_fk",
    }).onDelete("restrict"),
    foreignKey({ columns: [table.actorUserId], foreignColumns: [user.id], name: "campaign_session_encounter_firearm_attack_event_actor_fk" }).onDelete("restrict"),
    index("campaign_session_encounter_firearm_attack_event_history_idx").on(table.attackId, table.createdAt, table.id),
    check("campaign_session_encounter_firearm_attack_event_status_valid", sql`(${table.fromStatus} IS NULL OR ${table.fromStatus} IN ('aiming','trigger-ready','committed','fired-awaiting-timing','consequence-planned','requires-god-ruling','cancelled')) AND ${table.toStatus} IN ('aiming','trigger-ready','committed','fired-awaiting-timing','consequence-planned','requires-god-ruling','cancelled')`),
    check("campaign_session_encounter_firearm_attack_event_kind_nonblank", sql`length(trim(${table.eventKind})) > 0`),
    check("campaign_session_encounter_firearm_attack_event_metadata_object", sql`jsonb_typeof(${table.metadataJson}) = 'object'`),
  ],
);
