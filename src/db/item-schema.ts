...
export const item = pgTable(
  "items",
  {
    ...
    longestDimensionCm: doublePrecision("longest_dimension_cm"),
  ...
  (table) => [
    ...
    check("items_volume_finite", sql`${table.volumeL} is null or (${table.volumeL} >= 0 and ${table.volumeL} < 'Infinity'::float8)`),
    check("items_physical_form_valid", sql`${table.physicalForm} is null or ${table.physicalForm} in ('solid','liquid')`),
    check("items_dimension_finite", sql`${table.longestDimensionCm} is null or (${table.longestDimensionCm} >= 0 and ${table.longestDimensionCm} < 'Infinity'::float8)`),
...
export const weaponProfile = pgTable(
  "weapon_profiles",
  {
    ...
    reachDistance: doublePrecision("reach_distance"),
    shortRangeDistance: doublePrecision("short_range_distance"),
    mediumRangeDistance: doublePrecision("medium_range_distance"),
    longRangeDistance: doublePrecision("long_range_distance"),
    ...
    reloadType: text("reload_type"),
    readinessMode: text("readiness_mode"),
    drawInitiativeCost: doublePrecision("draw_initiative_cost"),
    readyInitiativeCost: doublePrecision("ready_initiative_cost"),
    reloadInitiativeCost: doublePrecision("reload_initiative_cost"),
    unloadInitiativeCost: doublePrecision("unload_initiative_cost"),
    firingModeChangeInitiativeCost: doublePrecision("firing_mode_change_initiative_cost"),
    ...
    ammunitionCyclingInitiativeModifier: doublePrecision("ammunition_cycling_initiative_modifier").default(0).notNull(),
    ammunitionRecoilResetInitiativeModifier: doublePrecision("ammunition_recoil_reset_initiative_modifier").default(0).notNull(),
  ...
  (table) => [
    ...
    check("weapon_profiles_capacity_rounds_valid", sql`${table.capacityRounds} IS NULL OR ${table.capacityRounds} > 0`),
    check("weapon_profiles_range_mode_valid", sql`${table.rangeMode} IS NULL OR ${table.rangeMode} IN ('melee','ranged','hybrid')`),
    check("weapon_profiles_distance_unit_valid", sql`${table.distanceUnit} IS NULL OR length(trim(${table.distanceUnit})) > 0`),
    check("weapon_profiles_reach_distance_valid", sql`${table.reachDistance} IS NULL OR ${table.reachDistance} > 0`),
    check("weapon_profiles_short_range_distance_valid", sql`${table.shortRangeDistance} IS NULL OR ${table.shortRangeDistance} > 0`),
    check("weapon_profiles_medium_range_distance_valid", sql`${table.mediumRangeDistance} IS NULL OR ${table.mediumRangeDistance} > 0`),
    check("weapon_profiles_long_range_distance_valid", sql`${table.longRangeDistance} IS NULL OR ${table.longRangeDistance} > 0`),
    check("weapon_profiles_range_order_valid", sql`(${table.shortRangeDistance} IS NULL OR ${table.mediumRangeDistance} IS NULL OR ${table.shortRangeDistance} <= ${table.mediumRangeDistance}) AND (${table.mediumRangeDistance} IS NULL OR ${table.longRangeDistance} IS NULL OR ${table.mediumRangeDistance} <= ${table.longRangeDistance})`),
    check("weapon_profiles_reload_type_valid", sql`${table.reloadType} IS NULL OR ${table.reloadType} IN ('Single','Magazine')`),
    check("weapon_profiles_readiness_mode_valid", sql`${table.readinessMode} IS NULL OR ${table.readinessMode} IN ('draw-is-ready','separate-ready-action')`),
    check("weapon_profiles_draw_cost_valid", sql`${table.drawInitiativeCost} IS NULL OR ${table.drawInitiativeCost} >= 0`),
    check("weapon_profiles_ready_cost_valid", sql`${table.readyInitiativeCost} IS NULL OR ${table.readyInitiativeCost} >= 0`),
    check("weapon_profiles_reload_cost_valid", sql`${table.reloadInitiativeCost} IS NULL OR ${table.reloadInitiativeCost} >= 0`),
    check("weapon_profiles_unload_cost_valid", sql`${table.unloadInitiativeCost} IS NULL OR ${table.unloadInitiativeCost} >= 0`),
    check("weapon_profiles_mode_change_cost_valid", sql`${table.firingModeChangeInitiativeCost} IS NULL OR ${table.firingModeChangeInitiativeCost} >= 0`),
  ],
);
...
export const weaponFiringMode = pgTable(
  "weapon_firing_modes",
  {
    ...
    sortOrder: integer("sort_order").notNull(),
    baseCyclingInitiativeCost: doublePrecision("base_cycling_initiative_cost"),
    baseRecoilResetInitiativeCost: doublePrecision("base_recoil_reset_initiative_cost"),
