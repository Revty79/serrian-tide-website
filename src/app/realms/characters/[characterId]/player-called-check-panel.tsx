...
export function PlayerCalledCheckPanel({ view }: { view: PlayerCalledCheckWorkspaceView }) {
  ...
  return <aside className={styles.panel} aria-labelledby="player-called-check-heading">
    ...
    <div className={styles.grid}>
      {view.calledChecks.map((request) => <article key={`check:${request.id}`} className={request.status === "pending" && request.recipientCharacterId === view.characterId ? styles.pending : ""}>
      ...
      {view.highLow.map((request) => <article key={`highlow:${request.id}`} className={request.status === "pending" && request.participantCharacterId === view.characterId ? styles.pending : ""}>
