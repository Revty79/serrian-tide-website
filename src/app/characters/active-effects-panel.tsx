...
export function ActiveEffectsPanel({ state, godMode, skillOptions = [], movementModes = [], disabled = false, onChange }: Props) {
  ...
  return <section className="active-effects-panel" aria-label="Conditions and Temporary Modifiers">
    ...
    <div className="active-effects-panel__columns">
      <section><h4>Conditions <span>{activeConditions.length}</span></h4>{activeConditions.length ? activeConditions.map((entry) => <article key={entry.id}><strong>{entry.name}</strong><span>{entry.duration.label}</span>{entry.description ? <p>{entry.description}</p> : null}<small>Source: {sourceLabel(entry.source)}</small>{godMode ? <button type="button" disabled={busy || disabled} onClick={() => void run(() => resolveConditionAction(state.characterId, entry.id))}>Resolve</button> : null}</article>) : <p className="active-effects-panel__empty">No active Conditions.</p>}</section>
      <section><h4>Temporary Modifiers <span>{activeModifiers.length}</span></h4>{activeModifierTotals.length ? <div className="active-effects-panel__totals" aria-label="Combined active modifier totals">{activeModifierTotals.map((entry) => <span key={`${entry.channel}:${entry.targetKey}`}><b>{modifierTargetLabel(entry.channel, entry.targetKey)}</b> {signed(entry.total)}</span>)}</div> : null}{activeModifiers.length ? activeModifiers.map((entry) => <article key={entry.id}><strong>{entry.label} <b>{signed(entry.amount)}</b></strong><span>{entry.channel} · {modifierTargetLabel(entry.channel, entry.targetKey)} · {entry.duration.label}</span><small>Source: {sourceLabel(entry.source)}</small>{godMode ? <button type="button" disabled={busy || disabled} onClick={() => void run(() => endModifierAction(state.characterId, entry.id))}>End</button> : null}</article>) : <p className="active-effects-panel__empty">No active Temporary Modifiers.</p>}</section>
    </div>
    {godMode && !disabled ? <div className="active-effects-panel__controls">
