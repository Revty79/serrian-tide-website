...
export function FirearmSetupPanel({ characterId, equipmentRevision, disabled }: { characterId: number; equipmentRevision: string; disabled: boolean }) {
  ...
  return <section className="equipment-state-panel__firearms" aria-label="Firearm equipment setup"><h4>Firearm setup</h4><p>Confirm each copy as empty, then ready a wielded firearm or attach a prepared magazine before combat.</p>
    {message ? <p role="status">{message}</p> : null}{view?.combatActive ? <p>Active combat: use its preparation controls to spend Initiative.</p> : null}
    ...
    {view?.firearms.map((entry) => { const blocked = disabled || busy || !view.canManage || view.combatActive, input = { characterId, instanceId: entry.instanceId, expectedVersion: entry.state?.version };
      return <fieldset key={entry.instanceId}><legend>{entry.name} · Copy #{entry.instanceId}</legend>
        {!entry.state ? <><label className="st-field">Initial firing mode<select className="st-control" value={modes[entry.instanceId] ?? entry.modes[0]?.id ?? ""} onChange={(event) => setModes({ ...modes, [entry.instanceId]: event.target.value })}>{entry.modes.map((mode) => <option key={mode.id} value={mode.id}>{mode.name}</option>)}</select></label>
          ...
          : <><p>{entry.state.loadedRounds} rounds · {entry.state.readied && !entry.state.needsRecovery ? "Ready" : "Needs preparation"}</p>
            ...
            {entry.reloadType === "Magazine" ? <><p>{entry.attachedMagazineInstanceId ? `Magazine copy #${entry.attachedMagazineInstanceId} attached. Removing it retains its rounds.` : "No magazine attached."}</p>
