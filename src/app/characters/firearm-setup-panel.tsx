        ...
        {!entry.state ? <><label className="st-field">Initial firing mode<select className="st-control" value={modes[entry.instanceId] ?? entry.modes[0]?.id ?? ""} onChange={(event) => setModes({ ...modes, [entry.instanceId]: event.target.value })}>{entry.modes.map((mode) => <option key={mode.id} value={mode.id}>{mode.name}</option>)}</select></label>
          ...
          : <><p>{entry.state.loadedRounds} rounds · {entry.state.needsRecovery ? "Needs cycling or recoil recovery" : entry.state.loadedRounds ? "Loaded" : "Empty"}</p>
            ...
            {entry.reloadType === "Magazine" ? <><p>{entry.attachedMagazineInstanceId ? `Magazine copy #${entry.attachedMagazineInstanceId} attached. Removing it retains its rounds.` : "No magazine attached. Choose a loaded copy below; loose cartridges must first be put into a magazine."}</p>
              ...
              <label className="st-field">Prepared magazine<select className="st-control" value={magazines[entry.instanceId] ?? ""} onChange={(event) => setMagazines({ ...magazines, [entry.instanceId]: event.target.value })}><option value="">Choose a loose, detached compatible copy</option>{entry.magazines.map((magazine) => <option key={magazine.instanceId} value={magazine.instanceId} disabled={!!magazine.attachedWeaponInstanceId || magazine.containerInstanceId !== null}>{magazine.name} · Copy #{magazine.instanceId} · {magazine.loadedRounds}/{magazine.capacity}{magazine.containerInstanceId !== null ? " · contained: move to Loose" : ""}</option>)}</select></label>
