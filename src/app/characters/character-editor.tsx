...
export function CharacterEditor({
  ...
  function changeItemQuantity(itemId: number, requestedQuantity: number) {
    ...
    if (getItemOwnershipStrategy(catalogItem.runtimeProfile, catalogItem.isFirearm === true || catalogItem.isMagazine === true || catalogItem.isContainer === true, catalogItem.powerResource) === "instance") {
      change((current) => ({
        ...current,
        ...
        itemInstances: resizeDraftOwnedItemInstances({
          ...
          requiresExactInstance: catalogItem.isFirearm === true || catalogItem.isMagazine === true || catalogItem.isContainer === true,
          powerResource: catalogItem.powerResource,
  ...
  function changeAdministrativeNumber(field: "fame" | "experience" | "totalExperience" | "quintessence" | "totalQuintessence" | "hpMultiplierSteps" | "baseMovementSteps" | "baseMagicSteps" | "creditsRemaining", value: number) {
    ...
    change((current) => ({
      ...
      profile: {
        ...current.profile,
        [field]: ["hpMultiplierSteps", "baseMovementSteps", "baseMagicSteps"].includes(field)
...
function IdentityTab({ draft, aggregate, selectedRace, disabled, godMode, raceLoading, onChange, onChooseRace }: { draft: CharacterDraft; aggregate: CharacterAggregate; selectedRace: CharacterRaceAggregate | null; disabled: boolean; godMode: boolean; raceLoading: boolean; onChange: (updater: (current: CharacterDraft) => CharacterDraft) => void; onChooseRace: (value: string) => Promise<void> }) {
  ...
  return <div className="character-section character-form-grid">
    ...
    <Field label="Race · Required"><select disabled={disabled || raceLoading} value={draft.profile.raceId ?? ""} onChange={(event) => void onChooseRace(event.target.value)}><option value="">Choose a Campaign Race</option>{aggregate.allowedRaces.map((entry) => <option key={entry.id} value={entry.id} disabled={entry.archived}>{entry.name}{entry.archived ? " · Archived" : ""}</option>)}</select></Field>
    ...
    {race ? <section className="character-race-card character-field--wide"><header><div><p>RACE RECORD</p><h3>{race.name}</h3></div></header><div className="character-race-summary"><div><span>Size</span><strong>{race.size || "Not recorded"}</strong></div><div><span>Base Magic</span><strong>{displayNumber(getCharacterBaseMagic(race.baseMagic, profile.baseMagicSteps))}</strong><small>Racial {race.baseMagic ?? "not recorded"} · Character +{displayNumber(getCharacterBaseMagicBonus(profile.baseMagicSteps))}</small></div><div className="is-wide"><span>Racial Quirk</span><strong>{race.racialQuirkName || "None recorded"}</strong><small>{[race.quirkSuccessEffect, race.quirkFailureEffect].filter(Boolean).join(" · ")}</small></div></div><div className="character-race-grid"><section><h4>Movement Modes</h4>{selectedRace.movementModes.length ? selectedRace.movementModes.map((mode) => <p key={mode.movementMode}>{mode.movementMode} · Base {displayNumber(getCharacterMovementBaseValue(mode.baseValue, profile.baseMovementSteps))} <small>Racial {displayNumber(mode.baseValue)} · Character +{displayNumber(getCharacterBaseMovementBonus(profile.baseMovementSteps))}</small>{mode.notes ? ` · ${mode.notes}` : ""}</p>) : <p>No movement modes recorded.</p>}</section><section><h4>Racial Skill Links</h4>{selectedRace.skillLinks.length ? selectedRace.skillLinks.map((link) => <p key={`${link.skillId}:${link.linkType}`}>{link.skillName} · {link.linkType}{link.value === null ? "" : ` ${displayNumber(link.value)}`}</p>) : <p>No racial Skill links recorded.</p>}</section></div></section> : null}
  </div>;
}
...
function EquipmentTab({ draft, aggregate, disabled, godMode, filter, search, purse, onFilter, onSearch, onQuantityChange, onRemoveInstance, campaignMoney }: { draft: CharacterDraft; aggregate: CharacterAggregate; disabled: boolean; godMode: boolean; filter: EquipmentFilter; search: string; purse: ReturnType<typeof getCampaignMoneyBreakdown>; onFilter: (filter: EquipmentFilter) => void; onSearch: (search: string) => void; onQuantityChange: (itemId: number, quantity: number) => void; onRemoveInstance: (draftId: number) => void; campaignMoney: (credits: number) => string }) {
  ...
  return <div className="character-section"><SectionHeading eyebrow="CAMPAIGN-AUTHORIZED CATALOG" title="Starting Equipment Store" detail={`${purse.formatted} ${godMode ? "currently held" : "remaining"}`} />{aggregate.campaign.currencySystem === "Derived Currency" ? <><div className="character-currency-ledger">{purse.entries.map((currency) => <div key={currency.id}><strong>{displayNumber(currency.quantity)} {currency.name}</strong><span>{currency.description || "Campaign currency"}</span></div>)}</div>{!purse.fullyRepresented ? <p className="character-notice">The configured denominations cannot exactly represent this balance.</p> : null}</> : null}<div className="character-equipment-toolbar"><Field label="Search permitted Items"><input type="search" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Name, ID, category, damage, armor, or type" /></Field><nav>{options.map(([value, label]) => <button key={value} type="button" className={filter === value ? "is-active" : ""} onClick={() => onFilter(value)}><span>{label}</span><strong>{aggregate.authorizedItems.filter((item) => matchesFilter(item, value)).length}</strong></button>)}</nav></div>{!godMode && ![...draft.items, ...draft.itemInstances].some((owned) => aggregate.authorizedItems.find((item) => item.id === owned.itemId)?.catalogScope.toLowerCase() === "equipment") ? <p className="character-notice">Purchase at least one Equipment item before completing Character creation. Inventory supplies alone do not satisfy starting equipment.</p> : null}<div className="character-equipment-list">{available.map((item) => {
    ...
    return <article key={item.id} className={quantity > 0 ? "is-owned" : ""}><div className="character-equipment-list__identity"><p>{item.canonicalId} · {item.recordType}</p><h3>{item.name}</h3><span>{item.category}{item.equipmentGroup ? ` · ${item.equipmentGroup}` : " · Inventory"}{item.isMagical ? " · Magical" : ""}</span>{item.description ? <small>{item.description}</small> : null}</div>{details.length ? <dl>{details.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl> : <p className="character-equipment-list__empty">No additional mechanics recorded.</p>}{item.weaponRulesText || item.armorRulesText ? <p className="character-equipment-list__rules">{item.weaponRulesText || item.armorRulesText}</p> : null}<div className="character-equipment-list__purchase"><div><span>Cost</span><strong>{item.credits === null ? "Not priced" : campaignMoney(item.credits)}</strong><small>{item.priceBasis}</small></div><label><span>{getItemOwnershipStrategy(item.runtimeProfile, item.isFirearm === true || item.isMagazine === true || item.isContainer === true, item.powerResource) === "instance" ? "Copies" : "Owned"}</span><input aria-label={`${item.name} Quantity`} type="number" min={0} step={1} disabled={disabled || (!godMode && item.credits === null)} value={quantity} onChange={(event) => onQuantityChange(item.id, numericValue(event.target.value))} /></label><button type="button" disabled={disabled || (!godMode && (item.credits === null || (item.credits > remaining && quantity === 0)))} onClick={() => onQuantityChange(item.id, quantity + 1)}>Buy One</button></div>{itemInstances.length ? <div className="character-item-instances">{itemInstances.map((owned, index) => { const persisted = owned.instanceId === null ? null : aggregate.itemInstances.find(({ id }) => id === owned.instanceId); const maximumCharges = item.powerResource?.maximumCharges ?? item.runtimeProfile.maximumCharges; const chargeDisplay = getItemChargeDisplay({ currentCharges: persisted?.currentCharges ?? maximumCharges ?? 0, maximumCharges }); return <div key={owned.draftId}><span>{owned.instanceId === null ? `New copy ${index + 1}` : `Copy #${owned.instanceId}`}</span><strong>{item.isContainer ? "Exact container copy" : item.isMagazine ? "Magazine copy (starts empty)" : item.isFirearm && item.runtimeProfile.useMode !== "charges" ? "Exact firearm copy" : chargeDisplay.label}</strong>{chargeDisplay.exceedsCurrentMaximum ? <small>Above the current template maximum; saved state preserved.</small> : null}<button type="button" disabled={disabled} onClick={() => onRemoveInstance(owned.draftId)}>Remove this copy</button></div>; })}</div> : null}</article>;
  })}{!available.length ? <p className="character-notice">No Campaign-authorized Items match this search.</p> : null}</div></div>;
}

function GodControlsTab({ draft, aggregate, selectedRace, purse, onNumberChange, onCurrencyChange }: { draft: CharacterDraft; aggregate: CharacterAggregate; selectedRace: CharacterRaceAggregate | null; purse: ReturnType<typeof getCampaignMoneyBreakdown>; onNumberChange: (field: "fame" | "experience" | "totalExperience" | "quintessence" | "totalQuintessence" | "hpMultiplierSteps" | "baseMovementSteps" | "baseMagicSteps" | "creditsRemaining", value: number) => void; onCurrencyChange: (currencyId: number, quantity: number) => void }) {
  const fields = [["fame", "Fame"], ["experience", "Available Experience"], ["totalExperience", "Lifetime Experience"], ["quintessence", "Available Quintessence"], ["totalQuintessence", "Lifetime Quintessence"], ["hpMultiplierSteps", "HP Multiplier Steps"], ["baseMovementSteps", "Base Movement Steps"], ["baseMagicSteps", "Base Magic Steps"]] as const;
  ...
  const baseMagic = getCharacterBaseMagic(selectedRace?.race.baseMagic, draft.profile.baseMagicSteps);
