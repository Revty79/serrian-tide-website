...
export async function getCharacter(characterId: number, godMode = false): Promise<CharacterAggregate> {
  ...
  const aggregate: CharacterAggregate = {
    ...
    authorizedItems: authorizedRows.map((entry) => ({
      ...
      rangeMode: entry.rangeMode as "melee" | "ranged" | "hybrid" | null,
      reachDistance: entry.reachDistance,
      shortRangeDistance: entry.shortRangeDistance,
      mediumRangeDistance: entry.mediumRangeDistance,
      longRangeDistance: entry.longRangeDistance,
