...
import type { RollTrayPrefill } from "./roll-tray";
...
export function CombatAidOperations({
  ...
  onOpenRollTray?: (prefill: RollTrayPrefill) => void;
}) {
  ...
  return <section className="combat-aid-operations">
    ...
    {selectedBinding && selectedPending ? <section className="combat-aid-resolution">
      <header><div><span>{selectedPending.status === "completed" ? "READY TO RESOLVE" : "AUTHORED ACTION"}</span><strong>{selectedPending.label}</strong></div><small>Source: {participant.identity.name} · Target: {boundTarget?.identity.name ?? "selection bound in runtime"} · Started {selectedPending.remainingInitiativeCost === 0 ? "complete" : `${selectedPending.remainingInitiativeCost} remaining`} · {selectedBinding.sourceRef}</small>{onOpenRollTray ? <button type="button" onClick={() => onOpenRollTray({ scope: "encounter", rollerCharacterId: characterId, targetCharacterId: boundTargetId, pendingActionId: selectedPending.id, purposeKind: selectedBinding.sourceKind === "weapon" || selectedBinding.sourceKind === "creature-attack" ? "attack" : "ability", label: selectedPending.label, rollType: "percentile" })}>Roll for Action</button> : null}</header>
    ...
    {unresolvedAttackBindings.length || participantReactions.length ? <section className="combat-aid-reactions">
      ...
      {participantReactions.map((reaction) => <div key={reaction.id}><span><b>{reaction.reactionType} · {reaction.status}</b><small>{reaction.committedInitiativeCost} committed{reaction.outcome ? ` · ${reaction.outcome}` : ""}</small></span>{onOpenRollTray && (reaction.status === "declared" || reaction.status === "needs-ruling") ? <button type="button" onClick={() => onOpenRollTray({ scope: "encounter", rollerCharacterId: characterId, reactionId: reaction.id, purposeKind: "defense", label: `${reaction.reactionType[0].toUpperCase()}${reaction.reactionType.slice(1)} Reaction`, rollType: "percentile" })}>Roll for Reaction</button> : null}{reaction.status === "declared" ? <><button type="button" disabled={busy} onClick={() => void perform(() => resolveEncounterReaction(encounterId, reaction.id, true), `${reaction.reactionType} succeeded and Initiative was reconciled.`)}>Success</button><button type="button" disabled={busy} onClick={() => void perform(() => resolveEncounterReaction(encounterId, reaction.id, false), `${reaction.reactionType} failed; committed cost remains.`)}>Failure</button></> : null}{reaction.status === "needs-ruling" ? <><button type="button" disabled={busy} onClick={() => void perform(() => ruleOnInterruptedEncounterReaction(encounterId, reaction.id, "keep"), "Committed Reaction cost kept by G.O.D. ruling.")}>Keep Cost</button><button type="button" disabled={busy} onClick={() => void perform(() => ruleOnInterruptedEncounterReaction(encounterId, reaction.id, "refund"), "Committed Reaction cost refunded without rewinding the timeline.")}>Refund Cost</button></> : null}</div>)}
    </section> : null}
