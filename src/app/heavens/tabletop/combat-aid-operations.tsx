...
export function CombatAidOperations({
  ...
  return <section className="combat-aid-operations">
    ...
    {unresolvedAttackBindings.length || participantReactions.length ? <section className="combat-aid-reactions">
      ...
      {participantReactions.map((reaction) => <div key={reaction.id}><span><b>{reaction.reactionType} · {reaction.status}</b><small>{reaction.committedInitiativeCost} committed{reaction.outcome ? ` · ${reaction.outcome}` : ""}</small></span>{onOpenRollTray && (reaction.status === "declared" || reaction.status === "needs-ruling") ? <button type="button" onClick={() => onOpenRollTray({ scope: "encounter", rollerCharacterId: characterId, reactionId: reaction.id, purposeKind: "defense", label: `${reaction.reactionType[0].toUpperCase()}${reaction.reactionType.slice(1)} Reaction` })}>Roll for Reaction</button> : null}{reaction.status === "declared" ? <><button type="button" disabled={busy} onClick={() => void perform(() => resolveEncounterReaction(encounterId, reaction.id, true), `${reaction.reactionType} succeeded and Initiative was reconciled.`)}>Success</button><button type="button" disabled={busy} onClick={() => void perform(() => resolveEncounterReaction(encounterId, reaction.id, false), `${reaction.reactionType} failed; committed cost remains.`)}>Failure</button></> : null}{reaction.status === "needs-ruling" ? <><button type="button" disabled={busy} onClick={() => void perform(() => ruleOnInterruptedEncounterReaction(encounterId, reaction.id, "keep"), "Committed Reaction cost kept by G.O.D. ruling.")}>Keep Cost</button><button type="button" disabled={busy} onClick={() => void perform(() => ruleOnInterruptedEncounterReaction(encounterId, reaction.id, "refund"), "Committed Reaction cost refunded without rewinding the timeline.")}>Refund Cost</button></> : null}</div>)}
    </section> : null}
