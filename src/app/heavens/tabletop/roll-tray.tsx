...
export function RollTray({
  ...
  return <section className="roll-tray" aria-label="Shared Serrian Tide Roll Tray" aria-busy={busy}>
    ...
    <div className="roll-tray-methods" aria-label="Roll method">
      <button type="button" className={`st-button${method === "random" ? " is-selected" : ""}`} onClick={() => setMethod("random")}>Random</button>
      <button type="button" className={`st-button${method === "entered" ? " is-selected" : ""}`} onClick={() => setMethod("entered")}>Enter physical</button>
    </div>

    <div className="roll-tray-grid">
      <label className="st-field"><span>Scope</span><select className="st-control" value={scope} onChange={(event) => setScope(event.target.value as RollTrayScope)}>
      ...
      <label className="st-field"><span>Purpose</span><select className="st-control" value={purposeKind} onChange={(event) => setPurposeKind(event.target.value as RollPurpose)}>{ROLL_PURPOSES.map((purpose) => <option key={purpose} value={purpose}>{purpose[0].toUpperCase() + purpose.slice(1)}</option>)}</select></label>
      ...
      {method === "entered" ? <label className="st-field"><span>Result</span><input className="st-control" type="number" min={1} max={100} step="1" value={enteredTotal} onChange={(event) => setEnteredTotal(event.target.value)} placeholder="73" /><small>Enter 100 for a physical percentile 00 result.</small></label> : <div className="roll-tray-random-note"><span>Result</span><strong>Generated securely by the server</strong><small>The browser cannot submit the random result.</small></div>}
      <label className="st-field"><span>Visibility</span><select className="st-control" value={visibility} onChange={(event) => setVisibility(event.target.value as RollVisibility)}><option value="god-only">G.O.D. Only</option><option value="private">Private: Roller + G.O.D.</option><option value="table">Show to Table</option></select></label>
      <label className="st-field"><span>Roller</span><select className="st-control" disabled={Boolean(prefill.weaponGovernance)} value={rollerCharacterId} onChange={(event) => setRollerCharacterId(event.target.value)}><option value="">No combatant</option>{characters.map((character) => <option key={character.characterId} value={character.characterId}>{character.name}</option>)}</select></label>
      <label className="st-field"><span>Target</span><select className="st-control" value={targetCharacterId} onChange={(event) => setTargetCharacterId(event.target.value)}><option value="">No target</option>{characters.map((character) => <option key={character.characterId} value={character.characterId}>{character.name}</option>)}</select></label>
      {scope === "encounter" ? <label className="st-field"><span>Action</span><select className="st-control" value={pendingActionId} onChange={(event) => { setPendingActionId(event.target.value); if (event.target.value) setReactionId(""); }}><option value="">No action link</option>{workspace.pendingActions.map((action) => <option key={action.id} value={action.id}>#{action.id} · {action.label} · {action.status}</option>)}</select></label> : null}
      {scope === "encounter" ? <label className="st-field"><span>Response</span><select className="st-control" value={reactionId} onChange={(event) => { setReactionId(event.target.value); if (event.target.value) setPendingActionId(""); }}><option value="">No response link</option>{workspace.reactions.map((reaction) => <option key={reaction.id} value={reaction.id}>#{reaction.id} · {reaction.reactionType} · {reaction.status}</option>)}</select></label> : null}
      <label className="st-field is-wide"><span>Roll name</span><input className="st-control" maxLength={200} value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Longsword attack, Stealth, G.O.D. reference…" /></label>
      {prefill.weaponGovernance ? <div className="roll-tray-governing-source"><span>Governing source</span><strong>{prefill.weaponGovernance.sourceLabel}</strong><small>{prefill.weaponGovernance.sourceDetail}</small><b>Original roll-over target: {prefill.weaponGovernance.originalTarget}%</b><em>The governing source is checked again when the Roll is recorded.</em></div> : <label className="st-field"><span>Roll-over target</span><input className="st-control" type="number" step="any" value={targetNumber} onChange={(event) => setTargetNumber(event.target.value)} placeholder="Leave blank for a free Roll" /><small>A supplied target is evaluated once.</small></label>}
      {!prefill.weaponGovernance && targetNumber !== "" ? <label className="st-field"><span>Target reason</span><input className="st-control" maxLength={200} value={targetReason} onChange={(event) => setTargetReason(event.target.value)} placeholder="G.O.D.-set target" /></label> : null}
      {prefill.weaponGovernance || targetNumber !== "" ? <label className="st-field"><span>Bonuses</span><textarea className="st-control" rows={2} value={bonuses} onChange={(event) => setBonuses(event.target.value)} placeholder={"Explicit bonus: 10"} /><small>One label and value per line. Bonuses lower the target.</small></label> : null}
      {prefill.weaponGovernance || targetNumber !== "" ? <label className="st-field"><span>Penalties</span><textarea className="st-control" rows={2} value={penalties} onChange={(event) => setPenalties(event.target.value)} placeholder="Explicit penalty: 20" /><small>One label and value per line. Penalties raise the target.</small></label> : null}
      ...
      <label className="st-field is-wide"><span>Notes</span><textarea className="st-control" rows={3} maxLength={2000} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Optional table context." /></label>
    </div>
    <footer><button type="button" className="st-button is-primary" disabled={busy || contextCompleted || method === "entered" && enteredTotal === ""} onClick={() => void submit()}>{busy ? "Recording…" : method === "random" ? "Roll" : "Record physical Roll"}</button><span>Linking this Roll does not change the action itself.</span></footer>
  </section>;
}
