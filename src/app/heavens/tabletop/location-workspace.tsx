...
import { GodShopVisitWorkspacePanel } from "./shop-visit-workspace";
...
export function SessionLocationPreparation({
  ...
  return <section className="tabletop-location-prep" aria-labelledby="session-location-prep-title">
    ...
    <div className="tabletop-location-prepared-grid">
      <section>
        ...
        {preparedTowns.length ? <div className="tabletop-location-pills">{preparedTowns.map((town) => <article key={town.id}>
          ...
          {canOperate && data.sessionEditable ? <button type="button" className="st-button is-secondary" disabled={busy} onClick={() => void run(() => removePreparedSessionTown(data.sessionId, town.id), `${town.name} was removed from Session preparation.`)}>Remove</button> : null}
      ...
      <section>
        ...
        {preparedShops.length ? <div className="tabletop-location-pills">{preparedShops.map((shop) => <article key={shop.id}>
          ...
          {canOperate && data.sessionEditable ? <button type="button" className="st-button is-secondary" disabled={busy} onClick={() => void run(() => removePreparedSessionShop(data.sessionId, shop.id), `${shop.name} was removed from Session preparation.`)}>Remove</button> : null}
    ...
    {canOperate && data.sessionEditable ? <div className="tabletop-location-selector-grid">
      <section>
        <header><div><span>CAMPAIGN TOWNS</span><h4>Prepare a Town</h4></div><input className="st-control" type="search" value={townSearch} placeholder="Find a Town" aria-label="Search Campaign Towns" onChange={(event) => setTownSearch(event.target.value)} /></header>
        <div className="tabletop-location-option-list">{availableTowns.map((town) => {
          const contents = activeTownContents(town);
          return <article key={town.id}><div><strong>{town.name}</strong><span>{town.category} · {contents.shops.length} Shops · {contents.places.length} places · {contents.npcs.length} NPCs</span></div><button type="button" className="st-button is-primary" disabled={busy} onClick={() => void run(() => prepareSessionTown(data.sessionId, town.id), `${town.name} is prepared for this Session.`)}>Prepare</button></article>;
        })}</div>
        {!availableTowns.length ? <p className="tabletop-empty">No available Towns match this search.</p> : null}
      </section>
      <section>
        <header><div><span>CAMPAIGN SHOPS</span><h4>Prepare an independent Shop</h4></div><input className="st-control" type="search" value={shopSearch} placeholder="Find a Shop" aria-label="Search Campaign Shops" onChange={(event) => setShopSearch(event.target.value)} /></header>
        <div className="tabletop-location-option-list">{availableShops.map((shop) => <article key={shop.id}><div><strong>{shop.name}</strong><span>{shop.category} · {shop.storefrontState}{shop.townName ? ` · ${shop.townName}` : " · Standalone"}</span></div><button type="button" className="st-button is-primary" disabled={busy} onClick={() => void run(() => prepareSessionShop(data.sessionId, shop.id), `${shop.name} is prepared for independent placement.`)}>Prepare</button></article>)}</div>
...
function TownPlacementComposer({
  ...
  return <section className="tabletop-location-composer">
    <header><div><span>TOWN PLACEMENT</span><h4>Add Town to Scene</h4></div><input className="st-control" type="search" value={search} placeholder="Search Towns" aria-label="Search Town placements" onChange={(event) => setSearch(event.target.value)} /></header>
    <label className="st-field"><span>Campaign Town</span><select className="st-control" value={townId ?? ""} disabled={busy} onChange={(event) => choose(event.target.value ? Number(event.target.value) : null)}><option value="">Choose a Town</option>{options.map((town) => <option value={town.id} key={town.id}>{town.name} — {town.category}</option>)}</select></label>
    ...
    <footer><button type="button" className="st-button is-primary" disabled={busy || !canOperate || !selected || data.selectedSceneId === null} onClick={() => void run("place")}>Add Town to Scene</button><button type="button" className="st-button is-secondary" disabled={busy || !canOperate || !selected || !data.sessionEditable} onClick={() => void run("create")}>Create Scene from Town</button></footer>
  </section>;
}
...
export function SceneLocationWorkspace({
  ...
  data: LocationPlacementWorkspace;
  visits: GodShopVisitWorkspace | null;
  canOperate: boolean;
}) {
  const router = useRouter();
  const preserveScroll = useInPlaceScrollPreservation();
  ...
  return <section className="tabletop-scene-locations" aria-labelledby="scene-location-directory-title">
    {visits ? <GodShopVisitWorkspacePanel data={visits} /> : null}
    ...
    {canOperate && (data.sceneEditable || data.sessionEditable) ? <div className="tabletop-location-controls">
      ...
      <section className="tabletop-location-composer">
        <header><div><span>SHOP PLACEMENT</span><h4>Add Shop to Scene</h4></div><input className="st-control" type="search" value={shopSearch} placeholder="Search Shops" aria-label="Search Shop placements" onChange={(event) => setShopSearch(event.target.value)} /></header>
        <label className="st-field"><span>Campaign Shop</span><select className="st-control" value={shopId ?? ""} disabled={busy || data.selectedSceneId === null} onChange={(event) => setShopId(event.target.value ? Number(event.target.value) : null)}><option value="">Choose a Shop</option>{shopOptions.map((shop) => <option key={shop.id} value={shop.id}>{shop.name} — {shop.category} — {shop.storefrontState}</option>)}</select></label>
        ...
        <footer><button type="button" className="st-button is-primary" disabled={busy || !shopId || data.selectedSceneId === null || !data.sceneEditable} onClick={() => void run(async () => { const selected = data.shops.find(({ id }) => id === shopId); const result = await addShopToScene(data.selectedSceneId!, shopId!); setShopId(null); setFeedback({ kind: "success", message: result.created ? `${selected?.name ?? "Shop"} was placed hidden.` : `${selected?.name ?? "Shop"} is already independently placed.` }); }, "Shop placed.")}>Add Shop to Scene</button></footer>
      </section>
    </div> : null}
    {data.selectedSceneId === null ? <p className="tabletop-empty">Create or select a Scene to place and reveal locations. Create Scene from Town remains available above.</p> : <div className="tabletop-location-directory">
      {data.sceneTowns.map((placement) => <article className="tabletop-town-placement" key={placement.town.id}>
        <header><div><span>{placement.town.category}{placement.town.archived ? " · Archived source" : ""}</span><h4>{placement.town.name}</h4><p>{placement.town.overview || "No Town overview."}</p></div><div><em className={placement.revealed ? "is-revealed" : "is-hidden"}>{placement.revealed ? "Town revealed" : "Hidden from players"}</em>{canOperate && data.sceneEditable ? <><button type="button" className="st-button is-secondary" disabled={busy} onClick={() => void run(() => setTownPlacementVisibility(data.selectedSceneId!, placement.town.id, !placement.revealed, !placement.revealed), placement.revealed ? `${placement.town.name} is hidden from players.` : `${placement.town.name} and all included content are revealed.`)}>{placement.revealed ? "Hide Town" : "Reveal Town + Included"}</button><button type="button" className="st-button is-secondary" disabled={busy || placement.town.archived} onClick={() => void previewRefresh(data.selectedSceneId!, placement.town.id)}>Preview Refresh</button><button type="button" className="st-button is-danger" disabled={busy} onClick={() => { if (window.confirm(`Detach ${placement.town.name} from this Scene? Campaign records and Character memberships will be preserved.`)) void run(() => detachTownFromScene(data.selectedSceneId!, placement.town.id), `${placement.town.name} was detached; Campaign records and Character references were preserved.`); }}>Detach</button></> : null}</div></header>
    ...
    {refreshPreview ? <div className="tabletop-location-refresh" role="dialog" aria-modal="true" aria-labelledby="town-refresh-title">
      <section><header><div><span>PLACEMENT REFRESH</span><h4 id="town-refresh-title">{refreshPreview.townName}</h4></div><button type="button" className="st-button is-secondary" onClick={() => setRefreshPreview(null)}>Close</button></header><p>Existing inclusion and visibility choices are preserved. New references are included but hidden. Removed builder relationships detach only their Town-derived placement references.</p>
        ...
        <footer><button type="button" className="st-button is-secondary" onClick={() => setRefreshPreview(null)}>Cancel</button><button type="button" className="st-button is-primary" disabled={busy || changes(refreshPreview) === 0} onClick={() => void run(async () => { await refreshTownPlacement(data.selectedSceneId!, refreshPreview.townId); setRefreshPreview(null); }, `${refreshPreview.townName} placement was refreshed.`)}>Apply {changes(refreshPreview)} changes</button></footer>
      </section>
    </div> : null}
  </section>;
}
