...
function Weapon({ draft, references, itemDirty, onChange }: { draft: ItemDraft; references: ItemAuthoringReferences; itemDirty: boolean; onChange: (draft: ItemDraft) => void }) {
  ...
  return <div className="item-section item-form-grid">
    ...
    {!ammunitionProfile ? <section className="item-firearm-timing item-field--wide">
      <SectionHeading eyebrow="FIREARM READINESS" title="Structured Runtime Authoring" />
      <p className="item-firearm-help">These exact whole-number fields are the only canonical values used by firearm readiness. Blank fields remain review-required; legacy text is never parsed or guessed. Zero explicitly means instantaneous.</p>
      <div className="item-form-grid">
        ...
        <Field label="Draw Initiative"><OptionalNumber value={profile.drawInitiativeCost} min={0} step={1} onChange={(drawInitiativeCost) => patch({ drawInitiativeCost })} /></Field>
        <Field label="Ready Initiative"><OptionalNumber value={profile.readyInitiativeCost} min={0} step={1} onChange={(readyInitiativeCost) => patch({ readyInitiativeCost })} /></Field>
        <Field label="Load / Reload Initiative"><OptionalNumber value={profile.reloadInitiativeCost} min={0} step={1} onChange={(reloadInitiativeCost) => patch({ reloadInitiativeCost })} /></Field>
        <Field label="Unload Initiative"><OptionalNumber value={profile.unloadInitiativeCost} min={0} step={1} onChange={(unloadInitiativeCost) => patch({ unloadInitiativeCost })} /></Field>
        <Field label="Change Mode Initiative"><OptionalNumber value={profile.firingModeChangeInitiativeCost} min={0} step={1} onChange={(firingModeChangeInitiativeCost) => patch({ firingModeChangeInitiativeCost })} /></Field>
      </div>
    </section> : null}
    {ammunitionProfile ? <section className="item-firearm-timing item-field--wide">
...
function Magazine({ draft, onChange }: { draft: ItemDraft; onChange: (draft: ItemDraft) => void }) {
  ...
  return <section className="item-section"><h3>Magazine Profile</h3><label className="st-field">Capacity (Rounds)<input className="st-control" type="number" min={1} step={1} required value={profile.capacityRounds} onChange={(event) => onChange({ ...draft, magazineProfile: { ...profile, capacityRounds: Number(event.target.value) } })} /></label>
    <Field label="Fill Initiative per Round"><OptionalNumber value={profile.fillInitiativeCostPerRound ?? null} min={0} step={1} onChange={(fillInitiativeCostPerRound) => onChange({ ...draft, magazineProfile: { ...profile, fillInitiativeCostPerRound } })} /></Field><p>Cost of inserting one round into a detached magazine during combat. Leave blank until authored. The weapon reload cost covers swapping magazines.</p>
