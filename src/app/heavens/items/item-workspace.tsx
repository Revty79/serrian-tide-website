...
function Preview({ draft }: { draft: ItemDraft }) {
  return <article className="item-preview">
    ...
    {draft.powers.filter((power) => power.customConstruction).map((power, index) => { const document = power.customConstruction!.document; const calculation = calculateSpell(document); const adapted = adaptSpellToMechanicalEffects(document); return <section key={`custom-magic-${index}`}><h4>{power.name || "Power"} Custom Magic</h4><p><strong>{document.name || "Untitled Construction"}</strong> · {document.tradition} · Calculator Mana {calculation.baseSpellManaCost} · Normal casting time {calculation.baseCombatCastingTime} Initiative</p><p>Item activation remains {power.initiativeCost ?? "unconfigured"} Initiative and {power.resourceCostKind === "none" ? "no resource cost" : `${power.resourceCostAmount ?? "?"} ${power.resourceCostKind}`}. Adapted effects: {adapted.valid ? adapted.effects.length : "manual review required"}.</p></section>; })}
