...
function Weapon({ draft, references, itemDirty, onChange }: { draft: ItemDraft; references: ItemAuthoringReferences; itemDirty: boolean; onChange: (draft: ItemDraft) => void }) {
  ...
  const firearmChecklist = !ammunitionProfile && isSupportedAmmunitionWeaponType(profile.weaponType) ? [
    ...
    { label: "2. Drawing, readying and reload costs", href: "#firearm-readying", missing: [profile.reloadType !== "Magazine" && profile.capacityRounds === null && "Capacity (Rounds)", !profile.readinessMode && "Readiness relationship", profile.drawInitiativeCost === null && "Draw Initiative", profile.readinessMode === "separate-ready-action" && profile.readyInitiativeCost === null && "Ready Initiative", profile.reloadInitiativeCost === null && "Load / Reload Initiative", profile.unloadInitiativeCost === null && "Unload Initiative", profile.firingModes.length > 1 && profile.firingModeChangeInitiativeCost === null && "Change Mode Initiative"].filter(Boolean) },
  ...
  return <div className="item-section item-form-grid">
    ...
    {!ammunitionProfile ? <section id="firearm-readying" className="item-firearm-timing item-field--wide">
      <SectionHeading eyebrow="WEAPON READINESS" title="Drawing, readying and reloading" />
      ...
      <div className="item-form-grid">
        ...
        <Field label="Readiness relationship"><select value={profile.readinessMode ?? ""} onChange={(event) => patch({ readinessMode: event.target.value === "draw-is-ready" || event.target.value === "separate-ready-action" ? event.target.value : null })}><option value="">Review required</option><option value="draw-is-ready">Drawing also readies</option><option value="separate-ready-action">Separate ready action</option></select></Field>
        ...
        <Field label="Ready Initiative"><OptionalNumber value={profile.readyInitiativeCost} min={0} step="any" onChange={(readyInitiativeCost) => patch({ readyInitiativeCost })} /></Field>
