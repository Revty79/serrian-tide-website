...
function Effects({ draft, skills, onChange }: { draft: ItemDraft; skills: ItemAuthoringReferences["skills"]; onChange: (draft: ItemDraft) => void }) {
  ...
  return <div className="item-section item-effects-editor">
    ...
    <div className="item-form-grid">
      ...
      {profile.useMode === "charges" ? <><Field label="Maximum Charges"><OptionalNumber value={profile.maximumCharges} min={1} step={1} onChange={(maximumCharges) => patchProfile({ maximumCharges })} /></Field><Field label="Charges Per Use"><OptionalNumber value={profile.chargesPerUse} min={1} step={1} onChange={(chargesPerUse) => patchProfile({ chargesPerUse })} /></Field><Field label="Recharge Rule / Notes" wide><textarea rows={4} value={profile.rechargeNotes} placeholder="Describe when the item recharges and how many Charges it restores." onChange={(event) => patchProfile({ rechargeNotes: event.target.value })} /></Field></> : null}
...
function Weapon({ draft, references, itemDirty, onChange }: { draft: ItemDraft; references: ItemAuthoringReferences; itemDirty: boolean; onChange: (draft: ItemDraft) => void }) {
  ...
  return <div className="item-section item-form-grid">
    ...
    <Field label="Reach"><input value={profile.reach} onChange={(e) => patch({ reach: e.target.value })} /></Field><Field label="Legacy Capacity Text"><input value={profile.capacity} onChange={(e) => patch({ capacity: e.target.value })} /></Field>
