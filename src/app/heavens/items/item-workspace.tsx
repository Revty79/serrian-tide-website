...
function Weapon({ draft, references, itemDirty, onChange }: { draft: ItemDraft; references: ItemAuthoringReferences; itemDirty: boolean; onChange: (draft: ItemDraft) => void }) {
  ...
  const firearmChecklist = !ammunitionProfile && isFirearmWeaponType(profile.weaponType) ? [
    ...
    { label: "3. Firing modes", href: "#firearm-firing-modes", missing: !profile.firingModes.length ? ["at least one firing mode"] : profile.firingModes.flatMap((mode) => mode.baseCyclingInitiativeCost === null || mode.baseRecoilResetInitiativeCost === null || !mode.deliveryCadence || !mode.roundsPerCadence ? [`${mode.name || "Unnamed mode"}: cycling cost, recoil recovery cost, delivery and rounds`] : []) },
  ] : null;
  return <div className="item-section item-form-grid">
    ...
    {firearmChecklist ? <section className="item-firearm-timing item-field--wide" aria-label="Firearm setup checklist"><h3>Get this firearm ready for combat</h3>
    ...
    {!ammunitionProfile ? <section id="firearm-readying" className="item-firearm-timing item-field--wide">
      <SectionHeading eyebrow="FIREARM READINESS" title="Drawing, readying and reloading" />
      ...
      <div className="item-form-grid">
        ...
        <Field label="Load / Reload Initiative"><OptionalNumber value={profile.reloadInitiativeCost} min={0} step="any" onChange={(reloadInitiativeCost) => patch({ reloadInitiativeCost })} /></Field>
