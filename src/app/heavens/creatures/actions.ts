...
export async function createDerivedCreature(parentCreatureId: number, variantName: string): Promise<CreatureAggregate> {
  const session = await requireGod();
  const parent = await getCreature(parentCreatureId);
  ...
  let canonicalId = `${parent.core.canonicalId}-${slug}`;
  let suffix = 2;
  while ((await db.select({ id: creature.id }).from(creature).where(eq(creature.canonicalId, canonicalId)).limit(1)).length) {
    canonicalId = `${parent.core.canonicalId}-${slug}-${suffix++}`;
  }
  const clone: CreatureDraft = {
    ...parent,
    id: undefined,
    core: {
      ...parent.core,
      ...
      canonicalName: name,
      parentCreatureId,
      parentCreatureName: parent.core.canonicalName,
      sourceSystem: null,
    },
    attributes: parent.attributes.map((row) => ({ ...row })),
    movement: parent.movement.map((row) => ({ ...row })),
    hpPools: parent.hpPools.map((row, index) => ({ ...row, canonicalId: `${canonicalId}-hp-${index + 1}` })),
    hitLocations: parent.hitLocations.map((row, index) => ({ ...row, hpPoolCanonicalId: row.hpPoolCanonicalId ? `${canonicalId}-hp-${Math.max(1, parent.hpPools.findIndex((pool) => pool.canonicalId === row.hpPoolCanonicalId) + 1)}` : null, sortOrder: index })),
    attacks: parent.attacks.map((row, index) => ({ ...row, canonicalId: `${canonicalId}-attack-${index + 1}` })),
    skillLinks: parent.skillLinks.map((row) => ({ ...row })),
    abilities: parent.abilities.map((row, index) => ({ ...row, canonicalId: `${canonicalId}-ability-${index + 1}` })),
    defenses: parent.defenses.map((row) => ({ ...row, seedIdentity: null })),
    uses: parent.uses.map((row) => ({ ...row, seedIdentity: null })),
    derivedCreatures: [],
  };
  void session;
  return saveCreature(clone);
}
