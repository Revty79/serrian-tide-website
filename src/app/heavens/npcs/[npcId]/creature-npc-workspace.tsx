...
function Special({ characterId, snapshot, disabled, onChange, onComplete }: { characterId: number; snapshot: CreatureDraft; disabled: boolean; onChange: (draft: CreatureDraft) => void; onComplete: () => void | Promise<void> }) {
  return <div className="creature-npc-section">
    <InteractionRulesEditor owner="creature" value={snapshot.core.interactionRules} onChange={(interactionRules) => onChange({ ...snapshot, core: { ...snapshot.core, interactionRules } })} /><SectionHeading eyebrow="SPECIAL MECHANICS" title="Traits & Abilities" action="Add Ability" onAction={() => onChange({ ...snapshot, abilities: [...snapshot.abilities, { canonicalId: `${snapshot.core.canonicalId}-npc-ability-${snapshot.abilities.length + 1}`, abilityName: "", abilityType: "", activation: "", requirements: "", usesRecharge: "", description: "", mechanicalEffect: "", notes: "", sortOrder: snapshot.abilities.length, crImpact: "None", effects: [] }] })} /><p className="creature-npc-runtime-note">Ability use is authoritative from the saved Current Snapshot. Save individual edits before resolving an ability.</p><div className="creature-npc-card-list">{snapshot.abilities.map((row, index) => <article key={row.canonicalId || index}><header><strong>{row.abilityName || `Ability ${index + 1}`}</strong><div><CreatureAbilityUseDialog sourceCharacterId={characterId} ability={row} disabled={disabled} onComplete={onComplete} /><button type="button" onClick={() => remove(snapshot, onChange, "abilities", index)}>Remove</button></div></header><div className="creature-npc-form-grid"><Field label="Name"><input value={row.abilityName} onChange={(e) => patch(snapshot, onChange, "abilities", index, { abilityName: e.target.value })} /></Field><Field label="Canonical ID"><input value={row.canonicalId} onChange={(e) => patch(snapshot, onChange, "abilities", index, { canonicalId: e.target.value })} /></Field><CreatureOriginField value={row.abilityType} onChange={(abilityType) => patch(snapshot, onChange, "abilities", index, { abilityType })} /><Field label="Activation Notes (Legacy Text)"><input value={row.activation} onChange={(e) => patch(snapshot, onChange, "abilities", index, { activation: e.target.value })} /></Field><Field label="CR Impact"><select value={row.crImpact} onChange={(e) => patch(snapshot, onChange, "abilities", index, { crImpact: e.target.value as CreatureCrImpact })}>{CREATURE_CR_IMPACTS.map((impact) => <option key={impact}>{impact}</option>)}</select></Field><Field label="Uses / Recharge"><input value={row.usesRecharge} onChange={(e) => patch(snapshot, onChange, "abilities", index, { usesRecharge: e.target.value })} /></Field><Field label="Requirements" wide><input value={row.requirements} onChange={(e) => patch(snapshot, onChange, "abilities", index, { requirements: e.target.value })} /></Field><Field label="Description" wide><textarea rows={3} value={row.description} onChange={(e) => patch(snapshot, onChange, "abilities", index, { description: e.target.value })} /></Field><Field label="Mechanical Notes (Legacy Text)" wide><textarea rows={3} value={row.mechanicalEffect} onChange={(e) => patch(snapshot, onChange, "abilities", index, { mechanicalEffect: e.target.value })} /></Field><Field label="Notes" wide><textarea rows={2} value={row.notes} onChange={(e) => patch(snapshot, onChange, "abilities", index, { notes: e.target.value })} /></Field></div><CreatureAbilityAuthoringEditor value={row.authoring} name={row.abilityName} onChange={(authoring) => patch(snapshot, onChange, "abilities", index, { authoring })} /><CreatureAbilityEffectsEditor ability={row} skillOptions={snapshot.skillLinks.map(({ skillId, skillName }) => ({ id: skillId, name: skillName }))} onChange={(ability) => patch(snapshot, onChange, "abilities", index, { ...ability, crImpact: row.crImpact })} /></article>)}</div><SectionHeading eyebrow="DESCRIPTIVE LEGACY DATA" title="Legacy Defense Notes" action="Add Defense" onAction={() => onChange({ ...snapshot, defenses: [...snapshot.defenses, { seedIdentity: null, defenseType: "", against: "", value: null, notes: "", sortOrder: snapshot.defenses.length, crImpact: "None" }] })} /><div className="creature-npc-row-list">{snapshot.defenses.map((row, index) => <div className="creature-npc-repeat creature-npc-defense" key={index}><input placeholder="Type" value={row.defenseType} onChange={(e) => patch(snapshot, onChange, "defenses", index, { defenseType: e.target.value })} /><input placeholder="Against" value={row.against} onChange={(e) => patch(snapshot, onChange, "defenses", index, { against: e.target.value })} /><input placeholder="Value" value={row.value ?? ""} onChange={(e) => patch(snapshot, onChange, "defenses", index, { value: e.target.value || null })} /><select value={row.crImpact} onChange={(e) => patch(snapshot, onChange, "defenses", index, { crImpact: e.target.value as CreatureCrImpact })}>{CREATURE_CR_IMPACTS.map((impact) => <option key={impact}>{impact}</option>)}</select><button type="button" onClick={() => remove(snapshot, onChange, "defenses", index)}>Remove</button></div>)}</div></div>;
}
