...
export function RaceAnatomyEditor({ value, onChange }: { value: RaceAnatomy | null; onChange: (value: RaceAnatomy | null) => void }) {
  ...
  return <section className={styles.editor} aria-label="Race HP and Hit Locations">
    <h3>HP &amp; Hit Locations</h3>
    <p>Define this Race&apos;s body using the same HP pools and 0–9 hit results as Creatures. Total HP still comes from each character&apos;s Constitution and HP Multiplier.</p>
    <GuidedField label="Body layout" className="st-field" help="Standard humanoid keeps the existing body table. Custom starts with that table so you can add a tail, wings, or other body regions. Changes take effect for characters assigned to this Race after saving. Existing damage and injury records are retained, including removed pools.">
      <select className="st-control" value={value ? "custom" : "humanoid"} onChange={(event) => onChange(event.target.value === "custom" ? createHumanoidRaceAnatomy() : null)}><option value="humanoid">Standard humanoid</option><option value="custom">Custom Race anatomy</option></select>
    </GuidedField>
    {value ? <>
      ...
      <div className={styles.list}>{value.hpPools.map((pool, index) => <article className={styles.pool} key={pool.canonicalId} aria-label={`HP Pool ${index + 1}`}>
        <GuidedField className="st-field" label="Pool Name" help="Name one shared HP pool, such as Tail. Renaming preserves its recorded damage."><input className="st-control" value={pool.poolName} onChange={(e) => changePool(index, { poolName: e.target.value })} /></GuidedField>
      ...
      <p>Repeated results can use one HP pool. Missing results and unmapped pools cannot receive automatic localized damage. Natural Protection is edited in Mechanics; its coverage follows these roll numbers.</p>
    ...
    </> : <p>This Race uses the existing humanoid pools and hit locations. Choose Custom Race anatomy to edit them.</p>}
  </section>;
}
