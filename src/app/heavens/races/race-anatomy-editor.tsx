  ...
  return <section className={styles.editor} aria-label={authoringOnly ? "Form HP and Hit Locations" : "Race HP and Hit Locations"}>
    ...
    {value ? <>
      ...
      <div className={styles.list}>{value.hpPools.map((pool, index) => <article className={styles.pool} key={pool.canonicalId} aria-label={`HP Pool ${index + 1}`}>
        <GuidedField className="st-field" label="Pool Name" help={authoringOnly ? "Name one shared HP pool, such as Tail. Renaming preserves the authored identity." : "Name one shared HP pool, such as Tail. Renaming preserves its recorded damage."}><input className="st-control" value={pool.poolName} onChange={(e) => changePool(index, { poolName: e.target.value })} /></GuidedField>
    ...
    </> : <p>{authoringOnly ? "This Form override uses standard humanoid pools and hit locations. Choose Custom Form anatomy to edit them." : "This Race uses the existing humanoid pools and hit locations. Choose Custom Race anatomy to edit them."}</p>}
  </section>;
}
