...
const guides: Record<string, PageHelp> = {
  race: { title: "Race authoring", introduction: "Define the racial information a Character uses without replacing the Character's own choices.", steps: authoring, fields: ["race", "interaction"], topics: { ...lifecycle,
    ...
    "Forms": "The Race itself is the normal state. Forms are alternate states available to this exact Race. Add, name, describe, annotate, remove and reorder Forms, then Save Race. Forms do not inherit from one another and are separate from Variants. Clone as Variant copies Forms independently. Forms are authoring data only; runtime transformation support is not implemented yet.",
