...
const guides: Record<string, PageHelp> = {
  race: { title: "Race authoring", introduction: "Define the racial information a Character uses without replacing the Character's own choices.", steps: authoring, fields: ["race", "interaction"], topics: { ...lifecycle,
    ...
    "Forms": "The Race itself is the normal state. Forms are alternate states available to this exact Race, with no inheritance between Forms. Use the collapsible sections to keep Race definitions or author Form Size, Anatomy, Movement, protection and attacks. A collection override replaces that category; an empty override means none. Skill additions preserve Race grants and learned Character Skills. Interaction Rules can add to or replace Race rules. Signed adjustments use the six existing Attribute scores; zero means no change. Speech, manipulation, equipment intent and restrictions are stored without runtime effects. Save Race after edits. Clone as Variant copies Forms and all their mechanics independently. Transformation defines entry/exit timing, costs, conditions, duration and limits without executing them. The Character Sheet View Form control previews the exact selected Race only; it never changes live state, saving, creation budgets or printing.",
