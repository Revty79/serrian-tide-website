...
export function getCharacterHpBreakdown(totalHp: number) {
  ...
  return {
    ...
    locations: CHARACTER_HUMANOID_HIT_LOCATIONS.map((location) => {
      const pool = map.get(location.poolKey)!;
      return { ...location, poolName: pool.name, hp: pool.hp };
    }),
  };
}
...
export function getMovementInitiative(dexterity: number, movementModeBaseValue: number): number {
  return getBaseInitiative(dexterity) * movementModeBaseValue;
}

export function normalizeSkillAttributeKey(value: string | null): CharacterAttributeKey | null {
  const key = value?.trim().toUpperCase();
  if (key === "CHA") return "CHR";
  return CHARACTER_ATTRIBUTE_KEYS.includes(key as CharacterAttributeKey) ? key as CharacterAttributeKey : null;
}
...
export function isSpellSkill(skill: CharacterSkillReference): boolean {
  const classification = skill.classification.trim().toLowerCase();
  return classification === "spell" || classification === "psionic skill" || classification === "reverberation";
}

export function getEffectiveSkillMaximum(skill: CharacterSkillReference, campaignMaximum: number): number {
  return isSpecialAbilitySkill(skill) ? SPECIAL_ABILITY_EFFECTIVE_MAXIMUM : campaignMaximum;
}

export function getRacialSkillGrant(race: CharacterRaceAggregate | null, skillId: number) {
...
export function getEffectiveSkillPoints(purchasedPoints: number, race: CharacterRaceAggregate | null, skillId: number) {
  return purchasedPoints + getRacialSkillGrant(race, skillId).minimum;
}

export function getPurchasedSkillMaximum(skill: CharacterSkillReference, campaignMaximum: number, racialPoints: number) {
  return Math.max(0, getEffectiveSkillMaximum(skill, campaignMaximum) - racialPoints);
}

export function getCreationPurchasedSkillMaximum(skill: CharacterSkillReference, maxStartingSkill: number, campaignMaximum: number, racialPoints: number) {
  return Math.min(maxStartingSkill, getPurchasedSkillMaximum(skill, campaignMaximum, racialPoints));
}

export function getSkillRank(pointsInvested: number, attributeModifier: number, parentRank: number | null, tier: number | null): number {
  if (!Number.isFinite(pointsInvested) || pointsInvested <= EPSILON) return 0;
  return tier !== null && tier > 1 ? (parentRank ?? 0) + pointsInvested : pointsInvested + attributeModifier;
}

export function getSkillRollTarget(attributeScore: number, skillRank: number) {
  return 100 - (attributeScore + skillRank);
}

export function getAttributePointsUsed(draft: CharacterDraft) {
  return CHARACTER_ATTRIBUTE_KEYS.reduce((total, key) => total + draft.attributes[key], 0);
}
...
export function getStartingFundsSpent(draft: CharacterDraft) {
  return draft.items.reduce((total, entry) => total + entry.quantity * entry.unitCostCredits, 0);
}

export function getStartingFundsRemaining(draft: CharacterDraft, startingCredits: number) {
...
export function getRecordedSpellLevel(skill: CharacterSkillReference): CharacterSpellAccessLevel | null {
  const level = skill.spellLevel?.trim().toLowerCase();
  return CHARACTER_SPELL_ACCESS_LEVELS.find((candidate) => candidate.name.toLowerCase() === level)?.name ?? null;
}
...
export function getCharacterMagicSystem(rootSkill: CharacterSkillReference): CharacterMagicSystem | null {
...
export function getSkillUnlockThreshold(rootSkill: CharacterSkillReference, campaignThreshold: number) {
  const systems = rootSystems(rootSkill);
  return systems?.some((system) => ONE_POINT_UNLOCK_SYSTEMS.has(system)) ? 1 : campaignThreshold;
}

export function isSkillAllowedByCampaign(skill: CharacterSkillReference, rootSkill: CharacterSkillReference, allowedSystems: readonly CampaignSystem[], raciallyGranted = false) {
  if (raciallyGranted) return true;
  if (skill.tier !== null && !allowedSystems.includes(`Tier ${skill.tier}` as CampaignSystem)) return false;
  const systems = rootSystems(rootSkill);
  return systems !== null && (systems.length === 0 || systems.some((system) => allowedSystems.includes(system)));
}

export function canAccessSpellAtLevel(skill: CharacterSkillReference, spellAccessLevel: CharacterSpellAccessLevel | null) {
  if (!isSpellSkill(skill)) return true;
  const spellLevel = getRecordedSpellLevel(skill);
  if (!spellLevel || !spellAccessLevel) return false;
  return CHARACTER_SPELL_ACCESS_LEVELS.findIndex((level) => level.name === spellLevel) <= CHARACTER_SPELL_ACCESS_LEVELS.findIndex((level) => level.name === spellAccessLevel);
}
...
export function getCharacterManaProfiles(
  ...
  const effectivePoints = (skill: CharacterSkillReference) => draft.skillAllocations
    .filter((allocation) => allocation.skillId === skill.id)
    .reduce((maximum, allocation) => Math.max(maximum, getEffectiveSkillPoints(allocation.points, race, skill.id)), getRacialSkillGrant(race, skill.id).minimum);

  return (Object.entries(MAGIC_SYSTEM_MANA_SKILLS) as Array<[CharacterMagicSystem, string]>).flatMap(([system, sourceSkillName]) => {
    const accessSkill = skillCatalog.find((skill) => getCharacterMagicSystem(skill) === system);
    if (!accessSkill || effectivePoints(accessSkill) <= EPSILON) return [];
    const sourceSkill = skillCatalog.find((skill) => skill.name.trim().toLowerCase() === sourceSkillName.toLowerCase());
    ...
    const next = CHARACTER_SPELL_ACCESS_LEVELS.find((level) => level.minimumMana > manaPool + EPSILON);
...
export function characterAggregateToDraft(aggregate: CharacterAggregate): CharacterDraft {
  ...
  return {
    ...
    profile,
    attributes,
    skillAllocations: aggregate.skillAllocations.map((allocation) => ({ draftId: allocation.id, skillId: allocation.skillId, parentDraftId: allocation.parentAllocationId, points: allocation.points })),
    items: aggregate.items.map((entry) => ({ itemId: entry.itemId, quantity: entry.quantity, unitCostCredits: entry.unitCostCredits })),
    currencyHoldings: aggregate.currencyHoldings.map((holding) => ({ currencyId: holding.currencyId, quantity: holding.quantity })),
  };
}
...
export function getCharacterSkillRanks(draft: CharacterDraft, skillCatalog: readonly CharacterSkillReference[], race: CharacterRaceAggregate | null = null) {
  ...
  const allocations = new Map(draft.skillAllocations.map((allocation) => [allocation.draftId, allocation]));
  ...
  function resolve(allocation: CharacterSkillAllocationDraft): number {
    ...
    const rank = getSkillRank(getEffectiveSkillPoints(allocation.points, race, allocation.skillId), attributeKey ? getAttributeModifier(attributeScore) : 0, parentRank, skill.tier);
...
export function evaluateCharacterReadiness(draft: CharacterDraft, aggregate: CharacterAggregate, race: CharacterRaceAggregate | null): CharacterReadiness {
  ...
  const identityComplete = Boolean(
    draft.name.trim() && draft.name.trim().toLowerCase() !== "new character" &&
    draft.profile.age !== null && draft.profile.sex.trim() &&
    (draft.profile.heightFeet ?? 0) * 12 + (draft.profile.heightInches ?? 0) > 0 &&
    draft.profile.weight !== null && draft.profile.weight > 0 &&
    draft.profile.skinColor.trim() && draft.profile.eyeColor.trim() && draft.profile.hairColor.trim() &&
    draft.profile.deity.trim() && draft.profile.definingMarks.trim() &&
    (aggregate.campaign.fatePointMethod !== "Rolled" || draft.profile.fatePoints !== null)
  );
  ...
  const attributesComplete = Math.abs(attributesUsed - aggregate.campaign.attributePoints) <= EPSILON && capsValid;
  if (Math.abs(attributesUsed - aggregate.campaign.attributePoints) > EPSILON) issues.push("Allocate the exact Campaign Attribute Point budget.");
  ...
  const allocationMap = new Map(draft.skillAllocations.map((allocation) => [allocation.draftId, allocation]));
  const relationshipKeys = new Set(aggregate.skillRelationships.filter((relationship) => relationship.relationshipType.toLowerCase() === "parent").map((relationship) => `${relationship.skillId}:${relationship.relatedSkillId}`));
  ...
  for (const allocation of draft.skillAllocations) {
    ...
    if (!skill || allocation.points < 0) { skillRulesValid = false; continue; }
    const racial = getRacialSkillGrant(race, allocation.skillId);
    if (allocation.points > getCreationPurchasedSkillMaximum(skill, aggregate.campaign.maxStartingSkill, aggregate.campaign.maxPointsInSkill, racial.minimum) + EPSILON) skillRulesValid = false;
    const key = `${allocation.parentDraftId ?? "root"}:${allocation.skillId}`;
    if (pathKeys.has(key)) skillRulesValid = false;
    pathKeys.add(key);
    let cursor = allocation;
    let root = skill;
    const visited = new Set<number>();
    while (cursor.parentDraftId !== null) {
      if (visited.has(cursor.draftId)) { skillRulesValid = false; break; }
      visited.add(cursor.draftId);
      const parent = allocationMap.get(cursor.parentDraftId);
      if (!parent || !relationshipKeys.has(`${cursor.skillId}:${parent.skillId}`)) { skillRulesValid = false; break; }
      const parentSkill = skillCatalog.get(parent.skillId);
      if (!parentSkill) { skillRulesValid = false; break; }
      if (parentSkill.tier !== null && skillCatalog.get(cursor.skillId)?.tier !== null && skillCatalog.get(cursor.skillId)?.tier !== (parentSkill.tier ?? 0) + 1) skillRulesValid = false;
      const threshold = getSkillUnlockThreshold(parentSkill, aggregate.campaign.pointsToUnlockNextTier);
      if (!racial.granted && getEffectiveSkillPoints(parent.points, race, parent.skillId) + EPSILON < threshold) skillRulesValid = false;
      cursor = parent;
      root = parentSkill;
    }
    if (!isSkillAllowedByCampaign(skill, root, aggregate.campaign.allowedSystems, racial.granted)) skillRulesValid = false;
    if (isSpellSkill(skill)) {
      const system = getCharacterMagicSystem(root);
      const profile = system ? getCharacterManaProfiles(draft, aggregate.skillCatalog, race).find((candidate) => candidate.system === system) : null;
      if (!profile || !canAccessSpellAtLevel(skill, profile.spellAccessLevel)) skillRulesValid = false;
    }
  }
  const skillsComplete = Math.abs(skillPointsUsed - aggregate.campaign.skillPoints) <= EPSILON && skillRulesValid;
  if (Math.abs(skillPointsUsed - aggregate.campaign.skillPoints) > EPSILON) issues.push("Allocate the exact Campaign Skill Point budget.");
  if (!skillRulesValid) issues.push("One or more Skill allocations violate Campaign rules.");

  const storyComplete = Boolean(draft.profile.personality.trim() && draft.profile.goals.trim() && draft.profile.secrets.trim() && draft.profile.backstory.trim() && draft.profile.motivations.trim());
  if (!storyComplete) issues.push("Complete every Story & Personality field.");

  const spent = getStartingFundsSpent(draft);
  const fundsRemaining = Math.max(0, aggregate.campaign.startingCreditAmount - spent);
  const authorized = new Map(aggregate.authorizedItems.map((entry) => [entry.id, entry]));
  const seen = new Set<number>();
  const itemRulesValid = draft.items.every((entry) => {
    const source = authorized.get(entry.itemId);
    if (!source || source.credits === null || seen.has(entry.itemId) || !Number.isInteger(entry.quantity) || entry.quantity <= 0 || Math.abs(source.credits - entry.unitCostCredits) > EPSILON) return false;
    seen.add(entry.itemId);
    return true;
  }) && spent <= aggregate.campaign.startingCreditAmount + EPSILON;
  if (!itemRulesValid) issues.push("Starting possessions must be priced and authorized by this Campaign.");
  const equipmentComplete = itemRulesValid && draft.items.some((entry) => authorized.get(entry.itemId)?.catalogScope.toLowerCase() === "equipment");
  if (!equipmentComplete) issues.push("Purchase at least one Campaign-authorized Equipment item.");

  return {
    ready: identityComplete && raceComplete && attributesComplete && skillsComplete && storyComplete && equipmentComplete,
    identityComplete, raceComplete, attributesComplete, skillsComplete, storyComplete, equipmentComplete,
    attributesUsed, skillPointsUsed, fundsRemaining, issues,
  };
}

export function getQuintessenceCost(purchaseType: "attribute" | "fatePoints" | "experience", quantity: number) {
  if (purchaseType === "attribute") return quantity * 2;
  if (purchaseType === "fatePoints") return quantity * 5;
  return quantity;
}
