  ...
  function prepare() {
    ...
    try {
      ...
      for (const creature of data.creatures) {
        ...
        if ((choice.killer || choice.value !== "") && !choice.reason.trim()) throw new Error(`${creature.name} needs the reason for the defeat credit or XP value ruling.`);
      ...
      for (const kill of data.kills) {
        ...
        if (!killerId || !data.playerIds.includes(killerId)) throw new Error("Credit a Player Character for " + kill.name + "'s defeat Fame.");
        if (kill.challengeRating === null) throw new Error(kill.name + " needs an authored CR before defeat Fame can be awarded.");
        if (!choice.reason.trim()) throw new Error("Record the reason for this defeat attribution.");
  ...
  return <details id="combat-closeout" className={styles.window} onToggle={(event) => setOpened(event.currentTarget.open)}><summary>{ended ? "Combat ended: XP & award history" : "End Combat & XP"}</summary>
    ...
    {data.npcs.map((npc) => { const choice = choices[npc.participantId] ?? blank; return <fieldset key={npc.participantId}><legend>{npc.name} / {npc.condition}{npc.awarded ? " / already awarded" : ""}</legend>
      {!npc.awarded ? <><label className={styles.check}><input type="checkbox" checked={choice.included} onChange={(event) => change(npc.participantId, { included: event.target.checked })} /> Include this NPC award</label>
        {choice.included ? <><p>G.O.D. chooses the amounts below for each selected Character. Creature defeat CR Fame is recorded separately.</p><div className={styles.fields}>
    ...
    {data.kills.length ? <fieldset><legend>Creature defeat Fame</legend><p>A Player Character earns the Creature&apos;s CR in Fame when they incapacitate or kill it. The same Creature awards XP and Fame only once; killing it later adds no second award. If credit is missing, choose the responsible Player below.</p>
      {data.kills.map((kill) => { const choice = killChoices[kill.participantId] ?? { included: false, killer: "", reason: "" };
        ...
        return <div key={kill.participantId}><p>{kill.name}: {kill.challengeRating ?? "CR missing"} Fame{kill.awarded ? " / already awarded" : ""}</p>
          {!kill.awarded ? <><label className={styles.check}><input type="checkbox" checked={choice.included} onChange={(event) => changeKill({ included: event.target.checked })} /> Credit this defeat and award Fame</label>
            {choice.included ? <div className={styles.fields}><label className="st-field">Player who defeated the Creature<select className="st-control" value={choice.killer || (kill.killerId ?? "")} onChange={(event) => changeKill({ killer: event.target.value })}><option value="">Choose the Player Character</option>{data.closeout.recipients.filter((entry) => data.playerIds.includes(entry.characterId)).map((entry) => <option value={entry.characterId} key={entry.characterId}>{entry.name}</option>)}</select></label>
            <label className="st-field">Defeat attribution reason<input className="st-control" value={choice.reason} onChange={(event) => changeKill({ reason: event.target.value })} /></label></div> : null}</> : null}
        </div>;
      })}</fieldset> : null}
