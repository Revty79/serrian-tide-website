...
export function IncomingEffectEvidence({ value, status }: { value: unknown; status?: string }) {
  ...
  return <div>
    {decisionRecorded ? <p>Original calculation required a G.O.D. ruling. A decision has been recorded.</p> : <p><strong>{resolution.status.replaceAll("-", " ")}</strong>{resolution.finalEffect
    ...
    <details><summary>Protection and interaction calculation</summary>
      <ol>{resolution.stages.map((stage) => <li key={stage.key}>
        <strong>{stage.key}</strong>: {stage.status}{stage.damageAfter !== null ? `; damage ${stage.damageAfter}` : ""}{stage.healingAfter ? `; healing ${stage.healingAfter}` : ""}
