...
export function EffectRuling({ encounterId, plan, disabled, closed, refresh }: { encounterId: number; plan: ActionEffectPlanView; disabled: boolean; closed: boolean; refresh: () => Promise<void> }) {
  ...
  return <details><summary>{plan.sourceSnapshot.displayName} · {plan.actorName} · {plan.status.replaceAll("-", " ")}</summary><p>{combatMessage(plan.explanation)}</p>
    ...
    {plan.effects.map((effect) => <fieldset key={effect.id}><legend>{effect.targetName} · {effect.effectType.replaceAll(".", " ")} · {effect.status.replaceAll("-", " ")}</legend><EffectEvidence value={effect.finalValue} />
