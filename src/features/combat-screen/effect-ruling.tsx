...
export function EffectRuling({ encounterId, plan, focusSequence, disabled, closed, refresh }: { encounterId: number; plan: ActionEffectPlanView; focusSequence?: number; disabled: boolean; closed: boolean; refresh: () => Promise<void> }) {
  ...
  const detail = useRef<HTMLDetailsElement>(null), attackRuling = useRef<HTMLDetailsElement>(null), recoveryRuling = useRef<HTMLDetailsElement>(null);
  useEffect(() => { if (focusSequence && detail.current) { detail.current.open = true; if (attackRuling.current) attackRuling.current.open = true; if (recoveryRuling.current) recoveryRuling.current.open = true; detail.current.scrollIntoView({ block: "center", behavior: "smooth" }); } }, [focusSequence]);
  const selectedTarget = Number(target) || (plan.targetSnapshot.length === 1 ? plan.targetSnapshot[0].participantId : 0);
  const hasAttackRuling = plan.sourceSnapshot.identity.startsWith("firearm-attack:") && plan.effects.some((effect) => effect.effectKey === "firearm-ruling-boundary" && !["applied", "manual-resolved", "declined"].includes(effect.status));
  ...
  return <details ref={detail}><summary>{plan.sourceSnapshot.displayName} · {plan.actorName} · {plan.status.replaceAll("-", " ")}</summary><p>{combatMessage(plan.explanation)}</p>
    ...
    {!["applied", "cancelled", "declined", "superseded"].includes(plan.status) ? <>
      <label className="st-field">{hasAttackRuling ? "Critical / attack ruling" : "Specific ruling / recovery reason"}<textarea aria-label={hasAttackRuling ? "Critical / attack ruling" : "Specific ruling / recovery reason"} className="st-control" value={reason} onChange={(event) => setReason(event.target.value)} /></label>
      ...
      {plan.effects.filter((effect) => effect.effectKey.startsWith("spell-combat-recovery:") && effect.targetParticipantId === selectedTarget && effect.status !== "manual-resolved").map((effect) => <div className={styles.actions} key={effect.id}>{(["revive", "remove-conditions"] as const).map((operation) => <button className="st-button" key={operation} disabled={disabled || closed || busy || !reason.trim() || (operation === "revive" ? authority.reviveHp === null : !authority.conditionRemoval)} onClick={() => void run(() => {
