...
type DefenseProps = { scope: CombatScreenScope; entity: CombatEntity; data: CombatScreenData; sources: Awaited<ReturnType<typeof readCombatCommandSources>> | null; disabled: boolean; refresh: () => Promise<void> };
function responseOptions({ data, entity }: DefenseProps) {
  return data.projection?.declarations.flatMap((declaration) => declaration.opportunities.filter((entry) => entity.responseOpportunityIds.includes(entry.id)).map((entry) => ({ ...entry, declaration }))) ?? [];
}
export function DefensePanel(props: DefenseProps) {
  const [opportunity, setOpportunity] = useState("");
  const opportunities = responseOptions(props);
  const selected = opportunities.find((entry) => String(entry.id) === opportunity) ?? (opportunities.length === 1 ? opportunities[0] : null);
  return <DefenseForm key={`${props.entity.participantId}:${selected?.id ?? "none"}`} {...props} opportunities={opportunities} selected={selected} setOpportunity={setOpportunity} />;
}
function DefenseForm({ scope, entity, data, sources, disabled, refresh, opportunities, selected, setOpportunity }: DefenseProps & {
  ...
  const [checked, setChecked] = useState<{ key: string; stateToken?: string; result: Awaited<ReturnType<typeof previewCombatDefense>> } | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void previewCombatDefense(scope, JSON.parse(fingerprint) as DefenseDeclarationInput)
        .then((result) => { if (active) { setChecked({ key: fingerprint, stateToken: data.projection?.stateToken, result }); setPreviewError(null); } })
