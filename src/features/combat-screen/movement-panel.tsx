...
export function MovementPanel({ scope, participantId, modes, disabled, hold, refresh }: { scope: CombatScreenScope; participantId: number; modes: readonly { movementMode: string }[]; disabled: boolean; hold: boolean; refresh: () => Promise<void> }) {
  ...
  return <div>{hold ? <><p className={styles.muted}>Hold preserves the opportunity to intervene under the Hold rules. Pass gives up the remaining opportunity for this round.</p><div className={styles.actions}>
    {(["hold", "pass"] as const).map((disposition) => <button className="st-button" key={disposition} disabled={disabled || busy} onClick={() => void run(() => scope.role === "player" ? setPlayerInitiativeDisposition(scope.characterId, scope.encounterId, disposition) : disposition === "hold" ? holdEncounterInitiative(scope.encounterId, participantId) : passEncounterInitiative(scope.encounterId, participantId), disposition === "hold" ? "Holding Initiative." : "Passed this round.")}>{disposition === "hold" ? "Hold Initiative" : "Pass this round"}</button>)}</div></> : <>
    ...
    {preview ? <><span>{preview.distance} feet · {preview.initiativeCost} Initiative</span><button className="st-button is-primary" disabled={disabled || busy} onClick={() => void run(() => {
