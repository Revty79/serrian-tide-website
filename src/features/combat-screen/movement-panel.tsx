...
export function MovementPanel({ scope, participantId, currentInitiative, stateToken, modes, disabled, hold, holding, refresh }: { scope: CombatScreenScope; participantId: number; currentInitiative: number; stateToken?: string; modes: readonly { movementMode: string }[]; disabled: boolean; hold: boolean; holding: boolean; refresh: () => Promise<void> }) {
  const [mode, setMode] = useState(""), [distance, setDistance] = useState(""), [flee, setFlee] = useState(false);
  const [message, setMessage] = useState(""), [busy, setBusy] = useState(false);
  const [checked, setChecked] = useState<{ key: string; value: Awaited<ReturnType<typeof previewCombatMovement>> } | null>(null);
  const request = useRef(""), running = useRef(false);
  const selectedMode = mode || modes[0]?.movementMode || "";
  const previewKey = JSON.stringify([participantId, selectedMode, distance, stateToken]);
  const preview = checked?.key === previewKey ? checked.value : null;
  const affordabilityIssue = preview ? initiativeAffordabilityIssue(preview.initiativeCost, currentInitiative) : null;
  function edited() { request.current = ""; setChecked(null); }
