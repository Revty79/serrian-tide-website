...
import { ammunitionSelectionInitiativeCost } from "@/features/items/projectile-combat";
import { projectileWeaponFamily } from "@/features/items/firearm-classification";
export function FirearmControls({ scope, entity, firearm, selectedModeId, inventory, disabled, refresh, preparationOnly = false }: { scope: CombatScreenScope; entity: CombatEntity; firearm: FirearmInstanceView; selectedModeId: number; inventory: MagazineInventoryView | null; disabled: boolean; refresh: () => Promise<void>; preparationOnly?: boolean }) {
  const guidance = firearmGuidance(firearm, selectedModeId);
  const family = projectileWeaponFamily(firearm.canonical.weaponType ?? "");
  const labels = family ? { ...preparationLabels, load: family === "bow" ? "Select arrow" : "Load / cock crossbow", reload: family === "bow" ? "Select arrow" : "Load / cock crossbow" } : preparationLabels;
  ...
  let preparationIssue = !state ? scope.role === "god" ? "Open Missing weapon mechanics ruling, enter an initialization reason, select a firing mode, then confirm this copy as empty and not readied." : "Ask the G.O.D. to confirm the initial state of this weapon copy."
  ...
  try {
    if (state && !preparationIssue) {
      if (operation === "draw" && firearm.equipmentState === "wielded") preparationIssue = "This weapon is already drawn.";
      else if (operation === "ready") preparationIssue = "Loaded weapons do not need a separate Ready action.";
      ...
      const timing = resolveFirearmPreparationTiming({ authored: { ...firearm.canonical, reloadInitiativeCost: ammunitionSelectionInitiativeCost(firearm.canonical), selectedMode }, ...input, ...(input.combineFollowUp ? { followUp: { requiresCycling: state.requiresCycling, requiresRecoilRecovery: state.requiresRecoilRecovery } } : {}) });
      ...
      if (magazineOperation && operation !== "unload") {
        ...
        preparationIssue ||= !replacement ? "Choose a magazine below. If none is listed, check owned magazine copies and the weapon profile's compatible magazine models; matching ammunition alone does not establish physical fit."
          : replacement.attachedWeaponInstanceId ? "That copy is already attached. Choose a detached magazine."
            : replacement.containerInstanceId !== null ? "Move this magazine to Loose in Inventory before attaching it to a firearm."
  ...
  return <section className={styles.notice} aria-labelledby={titleId}>
    ...
    <dl className={styles.attackFacts}>
      ...
      <div><dt>Weapon</dt><dd>{firearm.equipmentState !== "wielded" ? "Stowed" : "Drawn"}</dd></div>
      <div><dt>Your Initiative</dt><dd>{entity.currentInitiative}</dd></div>
    </dl>
    <p>{firearm.canonical.ammunitionName ?? "Ammunition needs configuration"} &middot; {firearm.inventoryAmmunitionQuantity} {family === "bow" ? "arrows" : family === "crossbow" ? "bolts" : "loose rounds"}</p>
    ...
    {guidance.catalogUpdate && state ? <div><p>Updated capacity: {firearm.canonical.capacityRounds} rounds. Applying it preserves loaded ammunition and completed preparation.</p>
    ...
    {guidance.needsPreparation || guidance.canFire ? <div className={styles.actions} aria-label="Weapon preparation choices">{(["draw", "reload", "recover-recoil"] as const).filter((entry) => entry === "reload" || entry === "draw" && firearm.equipmentState !== "wielded" || entry === "recover-recoil" && (state?.requiresCycling || state?.requiresRecoilRecovery)).map((entry) => <button key={entry} className="st-button" aria-pressed={operation === entry} onClick={() => setOperation(entry)}>{entry === "reload" && firearm.canonical.reloadType === "Magazine" ? "Load / swap magazine" : labels[entry]}</button>)}</div> : null}
    <details><summary>Other weapon handling</summary><label className="st-field">Preparation<select className="st-control" value={operation} onChange={(event) => setOperation(event.target.value as FirearmPreparationOperation)}>{FIREARM_PREPARATION_OPERATIONS.filter((entry) => entry !== "cycle" && entry !== "ready").map((entry) => <option key={entry} value={entry}>{labels[entry]}</option>)}</select></label></details>
    {guidance.needsPreparation || chosenOperation ? <>
      ...
      {magazineOperation && loadOperation ? <label className="st-field">Replacement magazine<select className="st-control" value={selectedMagazine} onChange={(event) => setMagazine(event.target.value)}><option value="">Choose a detached compatible copy</option>{firearm.magazines.map((entry) => <option key={entry.instanceId} value={entry.instanceId} disabled={!!entry.attachedWeaponInstanceId || entry.containerInstanceId !== null || entry.loadedRounds > 0 && entry.ammunitionItemId !== firearm.canonical.ammunitionItemId}>{entry.name} - Copy #{entry.instanceId} - {entry.loadedRounds}/{entry.capacity}{entry.attachedWeaponInstanceId ? " - attached" : entry.containerInstanceId !== null ? " - contained: move to Loose" : entry.loadedRounds === 0 ? " - empty: fill before firing" : entry.ammunitionItemId !== firearm.canonical.ammunitionItemId ? " - different ammunition" : ""}</option>)}</select></label> : null}
      {loadOperation && firearm.canonical.reloadType === "Single" ? <label className="st-field">{family === "bow" ? "Arrows to select" : family === "crossbow" ? "Bolts to load" : "Rounds to load"}<input className="st-control" type="number" min="1" step="1" value={rounds} onChange={(event) => setRounds(event.target.value)} /></label> : null}
    ...
    {scope.role === "god" ? <details open={!state}><summary>Missing weapon mechanics ruling</summary><div className={styles.fields}><label className="st-field">Preparation Initiative cost<input className="st-control" type="number" min="0" step="any" value={cost} onChange={(event) => setCost(event.target.value)} /></label><label className="st-field">Ruling reason<input className="st-control" value={reason} onChange={(event) => setReason(event.target.value)} /></label>
      {!state ? <label className="st-field">Missing capacity (rounds)<input className="st-control" type="number" min="1" value={capacity} onChange={(event) => setCapacity(event.target.value)} /></label> : null}</div>
      {!state ? <><p>This confirms the copy as empty.</p><button className="st-button" disabled={disabled || busy || !reason.trim()} onClick={() => void run({ firearm: firearm.itemInstanceId, selectedModeId, reason, capacity }, (idempotencyKey) => initializeFirearmState(scope.encounterId, { characterId: entity.participantId, itemId: firearm.itemId, itemInstanceId: firearm.itemInstanceId, selectedFiringModeId: input.targetFiringModeId ?? 0, reason, idempotencyKey, ...(capacity ? { capacityRuling: Number(capacity) } : {}) }))}>Confirm initial weapon state</button></> : null}
    </details> : null}
