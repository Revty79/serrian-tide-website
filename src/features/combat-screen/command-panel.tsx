...
export function CommandPanel({ scope, entity, data, command, setCommand, target: selectedTarget, setTarget, disabled, refresh }: { scope: CombatScreenScope; entity: CombatEntity; data: CombatScreenData;
  ...
  const choice: CombatChoice | null = source ? { participantId: entity.participantId, source, targetIds: targets,
    ...
    ...(source.kind === "weapon" ? { range: { attackMode: firearm ? "ranged" as const : draft.rangeMode, distance: draft.rangeDistance === "" ? null : Number(draft.rangeDistance), unit: draft.rangeUnit, beyondLongModifier: scope.role === "god" && draft.beyondLongModifier !== "" ? Number(draft.beyondLongModifier) : null, beyondLongReason: draft.beyondLongReason } } : {}),
  ...
  return <div><h3>{command}</h3>
    ...
    {command === "Weapons" ? <section aria-label="Combat weapons">
      ...
      {source?.kind === "weapon" ? <fieldset><legend>Weapon distance</legend><div className={styles.fields}><label className="st-field">Attack mode<select className="st-control" value={firearm ? "ranged" : draft.rangeMode} disabled={busy || !!firearm} onChange={(event) => edit({ rangeMode: event.target.value as "melee" | "ranged" })}><option value="melee">Melee / Reach</option><option value="ranged">Ranged / Short-Medium-Long</option></select></label><label className="st-field">Target distance<input className="st-control" type="number" min="0" step="any" value={draft.rangeDistance} onChange={(event) => edit({ rangeDistance: event.target.value })} /></label><label className="st-field">Distance unit<input className="st-control" value={draft.rangeUnit} placeholder="feet" onChange={(event) => edit({ rangeUnit: event.target.value })} /></label>{(!firearm && draft.rangeMode === "ranged") || firearm ? <><label className="st-field">Beyond Long modifier{scope.role === "god" ? <input className="st-control" type="number" step="any" value={draft.beyondLongModifier} placeholder="Required only beyond Long" onChange={(event) => edit({ beyondLongModifier: event.target.value })} /> : <span className="st-control">G.O.D. ruling required beyond Long</span>}</label>{scope.role === "god" ? <label className="st-field">Beyond Long ruling reason<input className="st-control" value={draft.beyondLongReason} onChange={(event) => edit({ beyondLongReason: event.target.value })} /></label> : null}</> : null}</div></fieldset> : null}
