...
export function useCombatCommandDrafts() {
  const [drafts, setDrafts] = useState<Record<string, Draft>>({});
  const submitted = useRef<Record<string, CombatSubmission>>({});
  const submissionFor = useCallback((key: string, create: () => CombatSubmission) => submitted.current[key] ??= create(), []);
  const forgetSubmission = useCallback((key: string) => { delete submitted.current[key]; }, []);
  return { drafts, setDrafts, submissionFor, forgetSubmission };
}
export function CommandPanel({ scope, entity, data, command, setCommand, target: selectedTarget, setTarget, disabled, refresh, draftState }: { scope: CombatScreenScope; entity: CombatEntity; data: CombatScreenData;
  draftState: ReturnType<typeof useCombatCommandDrafts>;
  ...
  const { drafts, setDrafts, submissionFor, forgetSubmission } = draftState;
  const [cache, setCache] = useState<Record<number, Sources>>({});
  const [messages, setMessages] = useState<Record<string, string>>({}), [busy, setBusy] = useState(false), [lastRequestKey, setLastRequestKey] = useState("");
  const message = messages[key] ?? "";
  const setMessage = useCallback((value: string) => setMessages((previous) => ({ ...previous, [key]: value })), [key]);
  const [checked, setChecked] = useState<{ key: string; stateToken?: string; sourceKey: string; value: Preview } | null>(null);
  ...
  const running = useRef(false), revision = useRef(0);
  ...
  const draft = { ...(stored ?? { ...blank, roll: scope.role === "god" ? { method: "digital" as const, value: "" } : emptyRoll }),
    source: stored?.source ?? automaticSource };
  ...
  const calledRequest = sources?.requests.find((entry) => entry.requestType === "called-shot" && ["pending", "clarification-requested", "approved"].includes(entry.status) && !entry.linkedDeclarationId && !entry.linkedFirearmAttackId && entry.sourceRef === source?.ref && entry.sourceInstanceId === source?.instanceId && entry.targetParticipantId === Number(target) && entry.frozenRequest.locationNumber === location?.number && entry.frozenRequest.objective === draft.objective.trim());
  const ruling = calledRequest?.status === "approved" ? calledRequest : null;
  ...
  const proposedMode = firearm ? "ranged" : weaponAttackMode(source?.rangeMode, draft.rangeMode);
  ...
  const choice: CombatChoice | null = source ? { participantId: entity.participantId, source, targetIds: targets,
    effectSelections: attackCommand ? {} : draft.applications,
  ...
  const fingerprint = JSON.stringify(choice), preview = !source?.unavailable && firearmReady && checked?.key === fingerprint && checked.stateToken === data.projection?.stateToken ? checked.value : null;
  ...
  function edit(change: Partial<Draft>) { setDrafts((values) => ({ ...values, [key]: { ...draft, ...change } })); forgetSubmission(key); }
  const previewReady = firearmReady && !!choice && !source?.unavailable && !["Hold", "Move", "Defend", "Weapons"].includes(command)
    && (command !== "Item" || !(currentItemMagic?.requiresGenericTarget || !groups.length) || itemDirectTargets.length > 0)
    && (command !== "Cast" && (command !== "Item" || !source?.ref.startsWith("item-power:")) || !!(command === "Cast" ? currentSpell : currentItemMagic) && groups.every((group) => group.kind === "aoe" || group.selected.length > 0))
    && (!["Attack", "Called Shot", "Ability"].includes(command) || targets.length > 0)
  ...
  }, [automaticSource, key, scope.role, setDrafts]);
  useEffect(() => {
    if (!previewReady) return;
    let active = true;
    const timer = setTimeout(() => {
      void previewCombatChoice(scope, JSON.parse(fingerprint) as CombatChoice).then((value) => {
        if (active) { setChecked({ key: fingerprint, stateToken: data.projection?.stateToken, sourceKey: previewSourceKey, value }); setPreviewError(null); }
  ...
  }, [scope, entity.participantId, data, setMessage]);
  useEffect(() => {
    let active = true;
    ...
    return () => { active = false; };
  }, [scope, entity.participantId, draft.source, source?.kind, source?.ref, setMessage]);
  useEffect(() => {
    let active = true;
  ...
  async function commit() {
    ...
    try {
      const submission = submissionFor(key, () => ({ choice, requestKey: crypto.randomUUID(), ...(needsRoll ? { roll: rollInput(draft.roll) } : {}) }));
      setLastRequestKey(submission.requestKey);
      await submitCombatChoice(scope, submission);
      // A confirmed submission is finished. Only an uncertain request keeps its
      // identity for retry; the next opportunity must create a new action/Roll.
      forgetSubmission(key);
      setDrafts((values) => ({ ...values, [key]: { ...(values[key] ?? draft), roll: { ...draft.roll, value: "" } } }));
      setMessage("Choice committed. Follow the shared prompt for what happens next.");
      await refresh();
  ...
  return <div data-combat-request-key={lastRequestKey}><h3>{command}</h3>
    ...
    {command === "Weapons" ? <section aria-label="Combat weapons">
    ...
    </section> : command === "Hold" || command === "Move" ? <MovementPanel key={entity.participantId} stateToken={data.projection?.stateToken} scope={scope} participantId={entity.participantId} currentInitiative={entity.currentInitiative} modes={sources?.movement ?? []} disabled={disabled || !entity.canControl || !entity.canActNow} hold={command === "Hold"} holding={entity.participationStatus === "holding"} refresh={refresh} /> : command === "Defend" ? <DefensePanel key={entity.participantId} scope={scope} entity={entity} data={data} sources={sources} disabled={disabled} refresh={refresh} /> : <>
      ...
      {command === "Item" && source && currentItemMagic && (currentItemMagic.requiresGenericTarget || !groups.length) ? <fieldset><legend>Who receives this Item&apos;s effect?</legend>
        <p>Choose yourself, another combatant, or everyone the Item&apos;s rule allows. Only the selected recipients receive its effects.</p>
        <button type="button" className="st-button" disabled={busy} onClick={() => { setTarget(String(entity.participantId)); edit({ targets: [], applications: {} }); }}>Use on myself</button>
      ...
      {scope.role === "player" ? ruling ? <p>G.O.D. approved: penalty {String(ruling.ruling.penalty)} · {String(ruling.ruling.reason)}</p> : calledRequest ? <p className={styles.notice}>Called Shot is {calledRequest.status === "pending" ? "waiting for the G.O.D. penalty ruling" : "awaiting your clarification"}. <button className="st-button" disabled={disabled || busy} onClick={() => void cancelPlayerCombatRulingRequest(scope.role === "player" ? scope.characterId : 0, scope.encounterId, calledRequest.id, "Player requested a fresh Called Shot ruling.").then(refresh).catch((error: unknown) => setMessage(combatMessage(error instanceof Error ? error.message : "Cancellation was not confirmed.")))}>Cancel Called Shot request</button></p> : <button className="st-button" disabled={disabled || busy || !source || !location || !draft.objective.trim()} onClick={async () => {
        if (!source || !location || running.current) return; running.current = true; setBusy(true);
        try { const identity = `${key}:ruling:${JSON.stringify([source.ref, source.instanceId, target, location.number, draft.objective])}`;
          const submission = submissionFor(identity, () => ({ choice: choice!, requestKey: crypto.randomUUID().replaceAll("-", "") }));
          await submitPlayerCombatRulingRequest(scope.characterId, scope.encounterId, { requestType: "called-shot", sourceKind: source.kind, sourceRef: source.ref, sourceInstanceId: source.instanceId, targetParticipantId: Number(target), intent: draft.objective, objective: draft.objective, locationNumber: location.number, idempotencyKey: submission.requestKey });
          forgetSubmission(identity);
      ...
      {command === "Attack" && source && !firearm ? <p className={styles.muted}>The last digit of your attack roll determines where you hit on the target. Damage from on-hit powers uses that same location. To choose where to aim, use Called Shot.</p> : null}
