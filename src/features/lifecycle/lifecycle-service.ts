...
function campaignDependencySpecs(campaignId: number): DependencySpec[] {
  return [
    ...
    { label: "Periodic health effects", blocking: false, query: sql<CountRow>`select count(*)::int as value from campaign_session_periodic_health_effect where campaign_id = ${campaignId}` },
    ...
    { label: "Duration bindings and Encounter rewards", blocking: false, query: sql<CountRow>`select ((select count(*) from campaign_session_effect_duration_binding where campaign_id = ${campaignId}) + (select count(*) from campaign_session_encounter_reward where campaign_id = ${campaignId}) + (select count(*) from campaign_session_encounter_reward_decision where campaign_id = ${campaignId}))::int as value` },
    { label: "Combat declaration checkpoints", blocking: false, query: sql<CountRow>`select count(*)::int as value from campaign_session_encounter_declaration_checkpoint where encounter_id in (select id from campaign_session_encounter where campaign_id = ${campaignId})` },
    ...
    { label: "Towns", blocking: false, query: sql<CountRow>`select count(*)::int as value from town where campaign_id = ${campaignId}` },
...
function scopedCampaignPredicate(
  ...
  if (scope === "encounter") return sql`encounter_id in (select id from campaign_session_encounter where campaign_id = ${campaignId})`;
  if (scope === "character") {
