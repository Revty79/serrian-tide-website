...
These are the selected authoritative routes; later passes reconcile the identified implementation gaps in place.
...
| Movement | Authoritative selected movement mode; distance with recorded timing input | Existing Initiative action and `getMaximumMovementDistance`; no pathfinding | Record completed segments as action evidence; replay original segment identity |
...
| Firearm preparation / shot | `firearm-readiness-service`, `firearm-attack-service`: exact instance/mode/target/ammunition | Existing authored preparation/trigger/cycling/recoil, reconciled with declaration checkpoint and Roll timing | Existing ammo ledger and per-bullet effect plan; original submission key/attack identity |
| Temporary effects | Owning Character modifier/condition services or exact direct occurrence | `initiative-capacity-service`, `initiative-runtime`, `duration-lifecycle-service`; effective changes at application/expiration | Bound duration progress only on actual Step/round/Scene boundaries; occurrence-local effects need equivalent lifecycle |
| Closeout / awards | `encounter-closeout-service`, `session-closeout-service` | Terminal declarations, timing, reactions, compatible effect-plan dispositions | Preserve resources and history; explicit reward record with duplicate protection |
