...
Creature master and individual editors share the **Interaction Rules** editor at the top of **Traits, Abilities & Defenses**. Existing editable Defense rows appear under **Legacy Defense Notes**. The Race editor uses the same component under **Attributes & Movement → Racial Interaction Rules**. No Natural Armor is added to Races. All new controls use shared semantic appearance variables and form classes.
