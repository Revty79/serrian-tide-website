...
- **Combat walkthrough, 2026-09-09 — crossover action choice (open; fix deferred by user).** When an ongoing action crosses another combatant's Initiative, that combatant should choose either an attempt to interrupt or a normal independent action, with their own legal target. Reaching Initiative does not itself mean responding to the original attacker. Current overlap opportunities require G.O.D. eligibility confirmation and route the combatant into defense/no-reaction, sometimes leaving a response unresolved and blocking progression. Fix the opportunity/choice handling and prompts together while preserving concurrent action timing. Initial estimate: 10,000–20,000 tokens including focused regression tests; account usage percentage is unknown. No engine change has been authorized during this walkthrough.
  - **Confirmed eligibility rule:** a combatant already committed to an unfinished action cannot respond, defend, interrupt, or start another normal action while that action is underway. Only a combatant who is not currently committed to an action may receive a response opportunity; being free does not automatically establish eligibility. Apply this in opportunity creation, server submission checks, and the UI. Busy combatants must not leave mandatory response prompts blocking progression. The independent action/interruption choice above applies only when the combatant is free to act.
  - Temporary testing workaround: for a busy combatant, or a free crossover combatant who is not reacting to this action, G.O.D. can choose **No, cannot respond** to dismiss that response opportunity. This does not spend their Initiative or mark them Passed. A busy combatant continues their existing action; a free combatant can choose their normal action when its controls are available. For a free combatant this records an ineligibility ruling, so it is only a workaround for the incorrect crossover prompt.
  - For an actual defense/interruption, choose **Yes, can respond** and let that combatant's controller use **Defend**. If eligibility was already allowed but they decline a response, use **no-reaction** and **Commit response**; this costs zero Initiative and requires no Roll, but records no defense against that action. If normal actions remain blocked, record the exact blocker; this workaround does not repair the underlying crossover flow.
- **Combat walkthrough, 2026-09-09 — insufficient Initiative at round end (open; deferred).** Bull 1 and Ysra each had 2 Initiative before being allowed to start attacks costing more than 2. At Round 1 / Initiative 0, the screenshot shows Bull's Horn / Headbutt with 2 remaining (expected finish -2), and Ysra's Longsword with 3 remaining. These actions should never have started with insufficient Initiative.
  - **Confirmed rule:** compare the selected action's cost with available Initiative before commitment. An action costing 2 or less is available with 2 points, including an affordable movement action. A more expensive action cannot start or partially spend those points. If no affordable action is chosen, retain the unused 2 Initiative and add it to the normal next-round Initiative allocation. Carry unused Initiative, not these unaffordable attacks as pending work. This finding concerns affordability when starting an action; additional costs imposed afterward have not been redefined.
  - Fix the declaration/command validation, affordable-choice explanation and Hold/round transition together. Verify both actors, an exact-cost action, rejection before resource spending, retained Initiative, and a fresh action choice next round. Initial estimate: 8,000–15,000 tokens including focused regression tests; no engine edit authorized yet.
  - Test-state limitation: pressing **Next round** currently retains already-started pending actions and their remaining costs. It does not undo the invalid commitments in this screenshot. Continuing this encounter therefore carries this known incorrect state.
