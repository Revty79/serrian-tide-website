...
## Pass 7 - recovery, XP, complete encounters and participation

Start: `683fe71`. Completed the retained Initiative persistence route, authoritative movement/flee intent, stale-request protection, explicit unapplied/partial consequence recovery, stranded checkpoint recovery, closeout blockers independent of declaration status, and all three agreed Creature XP modes plus additive full-per-recipient encounter XP. Reward decisions use exact occurrence/source identity and preserve attribution, eligibility and immutable receipts through retries, concurrent requests and repeated closeout.

The arrival/departure supplement is implemented in the same pass. The owning G.O.D. confirms escape, withdrawal and return through suspension and existing membership. New Player/NPC arrivals reuse Scene eligibility and late enrollment; new direct Creature requests are idempotent by exact occurrence. Completed outcomes and fired portions survive departure, future work/responses are reconciled with costs retained, and re-entry preserves resources and Initiative debt. Departure does not generate defeat/XP or a false Combat Step. Backend cards expose participation, reasons and inspectable state without a new screen.

Validation: **74 actual service scenarios** pass in a fresh disposable PostgreSQL cluster, including complete mixed-participant melee/movement/casting encounters in both request orders and complete firearm encounters. **1,261 unit cases in 145 files**, TypeScript, lint, production build, Drizzle check and whitespace check passed. The separate disposable account-deletion case passed with new reward-attribution references. Details and independently checked expected outcomes are in the [Pass 7 report](combat-completion-pass-7-2026-09-08.md).

The ordinary loopback `_dev` database already had migrations through 0044 when inspected. This pass applied only the pending additive **0045**. Migration before/after counts and value digests match across **32 existing tables**; the complete 46-migration ledger matches repository hashes. No campaign fixtures, reset or production deployment occurred.

The user subsequently directed explicit correction of historical plans 13 and 14. The tested recovery service applied Plan 13's approved 8 head damage to exact Dog occurrence -4 once and finalized Plan 14 with its existing declined damage. Both declarations now resolve with appended correction events, and the old force-end history remains. A fresh audit finds no unfinished plans. Six expected tables changed; the other 26 audited tables remain identical, including Initiative and resource/XP state. Generic cancellation now refuses unsettled completed outcomes; all-declined generated plans finish without an empty Apply request, and failed-Roll wording no longer implies an unrecorded defense.

End: `72b023c`. [Rules-to-service contract](../rules/combat-completion-contract.md) and [later screen handoff](../rules/combat-freeze-and-ui-handoff-2026-09-08.md) are updated. Backend passes are complete; replacement screens and human tabletop usability testing remain the subsequent phase.

## Follow-up: closed-state inspection, mechanical death and spell recovery

Continued from `6eb5b1d` on main tracking origin/main. Closed combat reads, mechanical fatal-head detection, distinct death/incapacity suspension, preserved XP eligibility and source-linked revival are described in the [correction report](combat-closed-state-and-recovery-correction-2026-09-08.md). The original seven-pass reports and ordinary campaign history are retained. This follow-up uses isolated fixtures and does not build replacement screens or migrate/correct ordinary campaign data.
